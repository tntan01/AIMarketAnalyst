...
class NewsService:
    ...
    def _fresh_context_entry(
        ...
        if not isinstance(entry, _MacroContextCacheEntry) or now >= entry.expires_at_utc:
            return None
    ...
    def latest_macro_context(
        ...
        *,
        ...
        with self._macro_context_cache_lock:
            # Support the former Phase 0 in-memory value shape while normal
            # writers use the typed, TTL-bound Phase 2 cache entry.
            legacy_key = f"{symbol}_{include_latest_statements}"
            if ai_service is not None:
                legacy_key += "_ai"
            legacy_cached = self._tier_scores_cache.get(legacy_key)
            if isinstance(legacy_cached, dict):
                safe_performance_call(performance_tracker, "increment", "macro_context_cache_hits")
            ...
            safe_performance_call(performance_tracker, "increment", "macro_context_cache_misses")
            with safe_performance_phase(performance_tracker, "macro_pair_build"):
