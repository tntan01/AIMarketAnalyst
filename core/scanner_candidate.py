"""Scanner candidate decision (Bước 08; target-only, not live-wired yet).

``build_candidate`` is the **single writer** of the candidate decision.  It
consumes only the Step 07 canonical output (``ScannerCompositionResult``) and:
...
from core.reason_codes import (
    ...
    EXECUTION_NOT_READY,
    ORDER_PREPARED,
    THRESHOLD_GAP_NOT_MET,
    THRESHOLD_POLICY_OPEN,
    THRESHOLD_RR_NOT_MET,
    THRESHOLD_SCORE_FLOOR_NOT_MET,
)
...
def _require_mapping(value: object, path: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        _error(path, "expected a mapping", code=SCANNER_SCHEMA_INVALID)
    return value
...
def _require_text(value: object, path: str) -> str:
    if type(value) is not str:
        _error(path, "expected a string", code=SCANNER_SCHEMA_INVALID)
    return value
...
def _require_datetime(value: object, path: str) -> datetime:
    if type(value) is datetime:
        return value
    if type(value) is not str:
        _error(path, "expected an ISO-8601 datetime", code=SCANNER_SCHEMA_INVALID)
    try:
        return datetime.fromisoformat(value)
    except ValueError as exc:
        _error(path, f"invalid ISO-8601 datetime: {exc}", code=SCANNER_SCHEMA_INVALID)
    return None  # pragma: no cover — _error raises


def _require_choice(value: object, choices: frozenset[str], path: str) -> str:
    if type(value) is not str or value not in choices:
        _error(path, f"expected one of {sorted(choices)}", code=SCANNER_SCHEMA_INVALID)
    return value
...
def _optional_int(
    ...
    if type(value) is not int or isinstance(value, bool):
        _error(path, "expected an integer or null", code=SCANNER_SCHEMA_INVALID)
    if minimum is not None and value < minimum:
        _error(path, f"expected at least {minimum}", code=SCANNER_SCHEMA_INVALID)
    if maximum is not None and value > maximum:
        _error(path, f"expected at most {maximum}", code=SCANNER_SCHEMA_INVALID)
    return value
...
def _optional_fraction(value: object, path: str) -> Fraction | None:
    ...
    if type(value) is str:
        try:
            ...
            _error(path, f"invalid fraction: {exc}", code=SCANNER_SCHEMA_INVALID)
...
def _require_positive_fraction(value: object, path: str) -> Fraction | None:
    ...
    if rr is not None and rr <= 0:
        _error(path, "must be positive", code=SCANNER_SCHEMA_INVALID)
    return rr
...
def _parse_reason_codes(value: object, path: str) -> tuple[str, ...]:
    if not isinstance(value, (list, tuple)):
        _error(path, "expected a list of reason codes", code=SCANNER_SCHEMA_INVALID)
    codes: list[str] = []
...
class ScannerOrderPayload:
    ...
    def __post_init__(self) -> None:
        ...
        versions = {
            ...
            "scoring_version": (self.scoring_version, SCANNER_SCORING_VERSION),
            "feature_version": (self.feature_version, SCANNER_V4_FEATURE_VERSION),
            "output_schema_version": (
                self.output_schema_version,
                SCANNER_OUTPUT_SCHEMA_VERSION,
            ),
            "snapshot_version": (self.snapshot_version, SCANNER_SNAPSHOT_VERSION),
            "safety_policy_version": (
        ...
        for field, (actual, grp) in versions.items():
            if type(actual) is not str or actual == "":
                _error(f"order_payload.{field}", "expected a non-empty string")
            expected_only = _ONLY_VERSIONS.get(field)
            if expected_only is not None and actual not in expected_only:
                _error(
    ...
    @classmethod
    def from_dict(cls, value: object, *, path: str = "order_payload") -> ScannerOrderPayload:
        expected = frozenset(
            {
                "symbol",
                "side",
...
_ONLY_VERSIONS = {
    "composition_version": (COMPOSITION_POLICY_VERSION, COMPOSITION_POLICY_LEGACY_VERSION),
...
def _require_positive_number(value: object, path: str) -> float:
    if type(value) is not float and type(value) is not int:
        _error(path, "expected a number", code=SCANNER_SCHEMA_INVALID)
    number = float(value)
    if number <= 0:
        _error(path, "expected a positive number", code=SCANNER_SCHEMA_INVALID)
    return number
...
class ScannerV4CandidateDecision:
    ...
    order_payload: ScannerOrderPayload | None

    def __post_init__(self) -> None:
        # --- identity / versions -------------------------------------------
        for field, expected in _ONLY_VERSIONS.items():
    ...
    def from_dict(
        ...
        return cls(
            ...
            order_payload=(
                None
                if order is None
                else ScannerOrderPayload.from_dict(order, path=f"{path}.order_payload")
            ),
        )
...
def _all_gates_pass(composition: ScannerCompositionResult) -> bool:
    return (
...
def _gate_codes_of(composition: ScannerCompositionResult) -> tuple[str, ...]:
    codes: list[str] = []
    for source in (
    ...
    *,
    ...
    if selected_side is not None and base_status != DATA_UNAVAILABLE:
        if composition.scenario.side not in (None, selected_side):
            return _unavailable(composition, thresholds, entry_confirmation, execution, proximity, CANDIDATE_SIDE_INCONSISTENT)
        if (
    ...
    def _base(
        *,
        ...
        order_payload: ScannerOrderPayload | None = None,
    ) -> ScannerV4CandidateDecision:
        return _assemble(
    ...
    #    OUT_OF_STRATEGY (the model has no such status).
    ...
    if not thresholds.certified():
        reason.append(THRESHOLD_POLICY_OPEN)
    ...
    if not floor_ok:
        reason.append(THRESHOLD_SCORE_FLOOR_NOT_MET)
    if not gap_ok:
        reason.append(THRESHOLD_GAP_NOT_MET)
    if not rr_ok:
        reason.append(THRESHOLD_RR_NOT_MET)
    ...
    confirmed_reason = [GATES_ALL_PASS, ENTRY_CONFIRMED]
    if entry_confirmation == "confirmed" and execution.can_execute:
        return _base(
            status=READY_NOW,
            reason=tuple(confirmed_reason + [EXECUTION_FRESH_OK, ORDER_PREPARED]),
    ...
    if entry_confirmation == "confirmed":
        return _base(
            ...
            reason=tuple(confirmed_reason + [EXECUTION_NOT_READY]),
        )
    if entry_confirmation == "unconfirmed":
        return _base(
            status=WAITING_CONFIRMATION,
            reason=(GATES_ALL_PASS, ENTRY_UNCONFIRMED),
        )
    return _base(
        status=WAITING_CONFIRMATION,
        reason=(GATES_ALL_PASS, ENTRY_CONFIRMATION_MISSING),
    )
