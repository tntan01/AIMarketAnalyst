...
SCANNER_SAFETY_AUDIT_VERSION = "scanner-safety-audit"
SCANNER_SAFETY_AUDIT_LEGACY_VERSION = "scanner-v4-safety-audit-v1"
...
def audit_safety_data(
    ...
    * no declaration at all → ``MISSING``.
    ...
    for category in SAFETY_AUDIT_CATEGORIES:
        ...
        for source in category_sources:
            if not source.point_in_time:
                reasons.append(SCANNER_SAFETY_AUDIT_NON_PIT)
            ...
            if source.observed_from <= pit_boundary <= source.observed_to:
                ...
                reasons.append(SCANNER_SAFETY_AUDIT_UNKNOWN)

        if eligible:
            status = AVAILABLE
        else:
            if not category_sources:
                reasons = [SCANNER_SAFETY_AUDIT_MISSING]
                status = MISSING
            elif status != NON_PIT:
    ...
    return SafetyDataAuditReport(
        audit_version=SCANNER_SAFETY_AUDIT_VERSION,
