"""Scanner V4 feature derivation (Path B — candle → technical raws).

This module is the V4 candle→raw producer that the V4 technical scorer consumes
...
It is a **documented port** of the V3 formulas from ``core/signal_engine.py``
(which is on the Bước 12 deletion list); formulas are copied verbatim so the V4
path stays live after V3 scoring is removed.

Governance (spec: ``docs/scanner/scanner-v4-features-spec.md``):
...
* **no V3 score dependency** — imports ``technical_context``, ``indicators`` and
  the retained canonical-SMC producer (``smc_scorer``/``smc_scoring_result``),
  NOT ``signal_engine``/``analysis_engine``/``analysis_pipeline``;
* raw ceilings are the V3 maxima, exactly matching the V4 component maxes
...
FEATURES_VERSION = "scanner-v4-features-v1"

# Raw ceilings — the V3 per-raw maxima, matching the V4 component maxes
...
def _choose_one(candidates: list[tuple[bool, int]]) -> int:
    """First TRUE candidate in list order (order is part of the V3 contract)."""
...
# the V3 functions until V3 scoring is deleted.
# ---------------------------------------------------------------------------
...
def derive_technical_raws(
    ...
    *,
    ...
    verbatim V3 ports.  ``smc`` is ``None`` unless ``canonical_smc`` (a retained
...
@dataclass(frozen=True, slots=True)
class TechnicalRawsV4:
    """The versioned V4 feature layer output for one symbol."""
    ...
    def __post_init__(self) -> None:
        if type(self.features_version) is not str or self.features_version != FEATURES_VERSION:
            raise TechnicalRawDerivationError(
