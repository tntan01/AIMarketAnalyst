"""Scanner row/API consumer contract (Bước 10; target-only).

10A replaces the legacy row adapter reads in ``core/scanner.py`` with a strict
reader that consumes ONLY the canonical Step 07/08 output — the
``ScannerCompositionResult`` / ``CanonicalPairSnapshot`` — and emits the full
...
* ``blocked_scanner_row_v4`` emits the full-schema row with
  ...
  ``macro_score = 15`` and zeroed scores for its legacy row);
* a legacy artifact or a missing/mismatched version is refused at read time, never
  coerced into a Scanner row.

This module is not wired to runtime.  The legacy executable scanner is untouched
...
# Legacy scored/legacy fields a Scanner row must never carry.
...
# Every key a row emits.  The row is *full-schema* — no legacy six-component
...
class RowContractError(ValueError):
    """Fail-closed error for the row reader (carries a reason code)."""
...
@dataclass(frozen=True, slots=True)
class ScannerRow:
    """Canonical-only scanner row (10A; strict, no legacy fallback)."""
...
def scanner_row_from_composition(
    composition: ScannerCompositionResult,
) -> ScannerRow:
    """Build the canonical-only scanner row for one composition result.
    ...
    re-scored and no legacy scored field is ever read.
    """
    ...
    # never zero-aggregated from legacy-style side totals.
...
def blocked_scanner_row_v4(
    ...
    *,
    ...
    """Full-schema ``DATA_UNAVAILABLE`` row (10A; no fabricated values).

    Deliberately NOT the legacy ``blocked_scanner_row`` shape
    (``core/scanner.py:355-402``) which hardcodes ``macro_score = 15`` and
    zeroed ``buy_score``/``sell_score``.  A blocked row keeps the full
...
def _blocked_identity() -> dict[str, str]:
    """Version identity for a blocked row (same locked constants)."""
    from core.scanner_v4_models import (
        SCANNER_V4_FEATURE_VERSION,
        SCANNER_MACRO_POLICY_VERSION,
        SCANNER_OUTPUT_SCHEMA_VERSION,
...
# ---------------------------------------------------------------------------
# Strict reader for an already-persisted row (target-only; legacy/missing
# identity is refused, never downgraded into a Scanner row).
# ---------------------------------------------------------------------------


def scanner_row_from_dict(value: object, *, path: str = "scanner_row") -> ScannerRow:
    """Strict deserializer of a persisted row.

    Refuses rows that are not stamped ``scanner-row`` (the current row version;
    the legacy ``scanner-v4-row-v1`` alias is still read for old artifacts) with
    the locked versions.  A legacy row (e.g. one from the executable ``core/scanner.py``
    whose ``row_version`` is missing) is refused — never reloaded as a current row.
    """
    if type(value) is not dict:
        raise RowContractError(
            SCANNER_SCHEMA_INVALID, path, "external payload must use a JSON object"
        )
    ...
    if "row_version" not in value:
        raise RowContractError(
            SCANNER_VERSION_MISSING, f"{path}.row_version", "missing row_version"
        )
    if value.get("row_version") not in (SCANNER_ROW_VERSION, SCANNER_ROW_LEGACY_VERSION):
        raise RowContractError(
            SCANNER_VERSION_MISMATCH,
            f"{path}.row_version",
    ...
    if missing:
        raise RowContractError(
            SCANNER_SCHEMA_INVALID, path, f"missing required fields: {missing}"
        )
    return ScannerRow(
        row_version=SCANNER_ROW_VERSION,
...
def _require_list(value: object, path: str) -> list[Any]:
    if not isinstance(value, list):
        raise RowContractError(SCANNER_SCHEMA_INVALID, path, "expected an array")
    return value


def _require_text(value: object, path: str, *, allow_empty: bool = False) -> str:
    if type(value) is not str:
        raise RowContractError(SCANNER_SCHEMA_INVALID, path, "expected a string")
    if not allow_empty and not value:
        raise RowContractError(SCANNER_SCHEMA_INVALID, path, "expected a non-empty string")
    return value


def _require_exact_version(value: object, expected: str, path: str) -> str:
    """Require the EXACT locked identity string (reject legacy/mixed/unknown)."""
...
def _optional_int(value: object, path: str) -> int | None:
    ...
    if type(value) is not int:
        raise RowContractError(SCANNER_SCHEMA_INVALID, path, "expected int or null")
    return value
...
def _parse_datetime(value: object, path: str) -> datetime:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise RowContractError(SCANNER_SCHEMA_INVALID, path, "must be timezone-aware")
