"""Scanner V4 backtest parity contract and artifact versions (Bước 09; target-only).
...
the artifact policy push *"V3 ... only read-only audit, non-replayable; không
được calibrate V4"*.

9A (parity) is structurally guaranteed by Bước 07: both adapters
(``build_live_snapshot`` / ``build_backtest_snapshot``) feed the **single**
``compose_scanner_v4`` API and differ only in ``capture_source`` provenance.
...
9C (artifacts) lives here too: the V4 artifact/ledger version identity.  The V3
backtest family keeps its own live versions (``phase0-backtest-safety-v1``,
``backtest-candidate-ledger-v1``, ...); V4 defines distinct locked versions so
no V3 artifact can ever be type-coerced into a V4 ledger, frozen strategy, or
replay.  A classifier marks every artifact as V4-replayable, V3-audit-only, or
incompatible.
"""
...
from core.reason_codes import (
    SCANNER_V4_BACKTEST_PARITY_VIOLATION,
...
# ---------------------------------------------------------------------------
# V4 artifact/ledger identity (9C).  Distinct from the V3 versions declared in
# core/backtest_contract.py / core/backtest_candidate_ledger.py / etc.
# ---------------------------------------------------------------------------

SCANNER_V4_BACKTEST_CONTRACT_VERSION = "scanner-backtest-contract-v4"
SCANNER_V4_CANDIDATE_LEDGER_VERSION = "scanner-v4-candidate-ledger-v4"
SCANNER_V4_CANDIDATE_REPLAY_VERSION = "scanner-v4-candidate-replay-v4"
SCANNER_V4_FROZEN_STRATEGY_VERSION = "scanner-v4-frozen-strategy-v4"
SCANNER_V4_BACKTEST_CONFIG_SCHEMA_VERSION = 10

# V3 artifact versions that classify as read-only/audit-only (never replayable,
# never usable to calibrate or activate V4).
...
V4_REPLAYABLE_ARTIFACT_KIND = "v4_replayable"
...
def classify_backtest_artifact(value: Any) -> ArtifactClassification:
    ...
    COMPLETE, strict V4 composition envelope — ``composition_version`` exactly
    ...
    * any V3 legacy version field value → ``v3_audit_only`` (read-only,
      non-replayable, never calibrates V4);
    * a full V4 composition envelope that parses strictly → ``v4_replayable``;
    * a dict that is not a parseable V4 composition (missing version, wrong
    ...
    * anything else (non-dict) → ``incompatible``.
    ...
    if type(value) is not dict:
        return ArtifactClassification(
            ...
            version_field="",
            version="",
            reason_codes=(SCANNER_V4_SCHEMA_INVALID,),
        )
    for version_field, version in value.items():
        ...
        if version in V3_LEGACY_ARTIFACT_VERSIONS:
            return ArtifactClassification(
                ...
                version_field=version_field,
                version=version,
                reason_codes=(SCANNER_V4_LEGACY_V3_AUDIT_ONLY,),
            )

    # A replayable V4 artifact must declare the exact canonical composition
    # identity — not merely carry one matching V4-family field.
    if value.get("composition_version") != COMPOSITION_POLICY_VERSION:
    ...
    try:
        ScannerV4CompositionResult.from_dict(value)
    except ValueError as exc:
        code = getattr(exc, "code", None)
        if code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY:
            return ArtifactClassification(
                kind=V3_AUDIT_ONLY_ARTIFACT_KIND,
                version_field="composition_version",
                version=str(value.get("composition_version")),
        ...
        reason_code = code if code in (_V4_INCOMPATIBLE_CODES) else SCANNER_V4_SCHEMA_INVALID
        return ArtifactClassification(
            kind=INCOMPATIBLE_ARTIFACT_KIND,
            version_field="composition_version",
            version=str(value.get("composition_version")),
    ...
    return ArtifactClassification(
        kind=V4_REPLAYABLE_ARTIFACT_KIND,
        version_field="composition_version",
...
_V4_INCOMPATIBLE_CODES = frozenset({
    SCANNER_V4_VERSION_MISMATCH,
    SCANNER_V4_VERSION_MISSING,
    SCANNER_V4_SCHEMA_INVALID,
})
...
def require_v4_replayable(value: dict[str, object]) -> dict[str, object]:
    """Return value unchanged when it is V4-replayable; otherwise raise.

    Fail-closed: any V3/missing/mismatched identity refuses replay before any
    ...
    if verdict.kind == V4_REPLAYABLE_ARTIFACT_KIND:
        return value
    raise ScannerV4ArtifactError(verdict)


class ScannerV4ArtifactError(ValueError):
...
class BacktestParityReport:
    ...
    def reason_codes(self) -> tuple[str, ...]:
        return () if self.passed else (SCANNER_V4_BACKTEST_PARITY_VIOLATION,)
...
def verify_composition_parity(
    live: ScannerV4CompositionResult,
    backtest: ScannerV4CompositionResult,
    ...
    return BacktestParityReport(
        contract_version=SCANNER_V4_BACKTEST_CONTRACT_VERSION,
        passed=not diffs,
        diffs=tuple(diffs),
    )
