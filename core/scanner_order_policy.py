"""Scanner runtime order policy — single owner config seam (Bước 12/13).
...
class RuntimeOrderPolicy:
    ...
    def from_dict(cls, value: object) -> "RuntimeOrderPolicy":
        ...
        if version not in (ORDER_POLICY_VERSION, ORDER_POLICY_LEGACY_VERSION):
            raise OrderPolicyError(
        ...
        safety = SafetyPolicy(policy_version=SCANNER_SAFETY_POLICY_VERSION)
        if raw_safety is not None:
            ...
            if sv not in (SCANNER_SAFETY_POLICY_VERSION, SCANNER_LEGACY_SAFETY_POLICY_VERSION):
                raise OrderPolicyError(
                    "safety.policy_version", f"expected {SCANNER_SAFETY_POLICY_VERSION!r}"
                )
...
DEFAULT_ORDER_POLICY_FILENAME = "scanner_order_policy.json"
...
def load_runtime_order_policy(
    ...
    Reads ``config/scanner_order_policy.json`` by default. Any failure
