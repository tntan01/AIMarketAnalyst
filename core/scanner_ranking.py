"""Scanner ranking (Bước 08; target-only, not live-wired yet).
...
* the tally NEVER reads news/spread/macro — there is no penalty and no Macro
  tie-break, and no legacy ``opportunity_score`` concept exists;
...
from core.scanner_v4_models import (
    ...
    DATA_UNAVAILABLE,
    READY_NOW,
    SCANNER_LEGACY_RANKING_VERSION,
...
def _require_runtime_policy(policy: ScannerRankingPolicy) -> ScannerRankingPolicy:
    ...
    """
    if policy.ranking_version not in (SCANNER_V4_RANKING_VERSION, SCANNER_LEGACY_RANKING_VERSION):
        raise RankPolicyError(
...
def rank_scanner_candidates(
    ...
    *,
    policy: ScannerRankingPolicy | None = None,
...
def grouped_scanner_candidates(
    candidates: list[ScannerV4CandidateDecision],
    *,
    policy: ScannerRankingPolicy | None = None,
