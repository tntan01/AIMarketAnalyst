"""Deterministic, target-only Scanner analysis pipeline and direct composition.

Step 07 of the Scanner migration implements the *analysis pipeline* that
...
* ONE composition API ``compose_scanner`` shared by live and backtest;
...
This module is not wired to runtime.  The legacy runtime is untouched until the
atomic cutover; the legacy paths to remove are ledgered in Mục 7.2/7.3.
"""
...
COMPOSITION_POLICY_VERSION = "scanner-composition"
COMPOSITION_POLICY_LEGACY_VERSION = "scanner-composition-v4"
...
class ScenarioPlan:
    ...
    ``entry_zone_low``/``entry_zone_high`` carry the REAL protective-zone band the
    entry is anchored to (both-or-neither, optional).  They are present only for
    plans built from real structure so the UI can draw the entry as the true zone
    rectangle instead of a single level; synthetic/None plans omit them and the
    strict reader keeps accepting the legacy 5-key form.
    """
    ...
    def __post_init__(self) -> None:
        ...
        # Optional protective-zone band: both-or-neither, strictly ordered, and it
        # must actually contain the anchored entry (entry sits ON one band edge).
        low = self.entry_zone_low
        high = self.entry_zone_high
        if (low is None) != (high is None):
            raise CompositionInputError(
                "scenario_plan.entry_zone",
                "entry_zone_low and entry_zone_high must be provided together",
            )
        if low is not None:
            low_f = _require_positive(low, "scenario_plan.entry_zone_low")
            high_f = _require_positive(high, "scenario_plan.entry_zone_high")
            if not (low_f < high_f):
                raise CompositionInputError(
                    "scenario_plan.entry_zone",
                    "entry zone requires low < high",
                )
            if not (low_f <= entry <= high_f):
                raise CompositionInputError(
                    "scenario_plan.entry_zone",
                    "anchored entry must lie within the protective-zone band",
                )
            object.__setattr__(self, "entry_zone_low", low_f)
            object.__setattr__(self, "entry_zone_high", high_f)
    ...
    def to_canonical_dict(self) -> dict[str, Any]:
        # The band is emitted only when both halves are present; absent bands keep
        # the exact legacy 5-key shape so old payloads round-trip unchanged.
...
def _scenario_plan_from_dict(value: object, *, path: str) -> ScenarioPlan:
    payload = _require_mapping(value, path)
    # Legacy 5-key form OR the 7-key form carrying the protective-zone band.  The
    # band is both-or-neither, so a payload is valid iff the required base keys are
    # present and any extra keys are exactly the band pair.  Neither is required to
    # be present, so old rows (5-key) still read after the band was introduced.
    required = {"direction", "entry", "stop_loss", "take_profit", "source"}
    band = {"entry_zone_low", "entry_zone_high"}
    keys = frozenset(payload)
    if not required <= keys:
        raise CompositionInputError(
            path, f"missing scenario plan keys: {sorted(required - keys)}"
        )
    if not keys <= (required | band):
        raise CompositionInputError(
            path, f"unexpected scenario plan keys: {sorted(keys - (required | band))}"
        )
    if (("entry_zone_low" in keys) != ("entry_zone_high" in keys)):
        raise CompositionInputError(
            path, "entry_zone_low and entry_zone_high must be provided together"
        )
    ...
    if "entry_zone_low" in keys:
        plan_kwargs["entry_zone_low"] = _require_positive_number(
            f"{path}.entry_zone_low", payload["entry_zone_low"]
        )
        plan_kwargs["entry_zone_high"] = _require_positive_number(
            f"{path}.entry_zone_high", payload["entry_zone_high"]
        )
    return ScenarioPlan(**plan_kwargs)
...
def snapshot_id_of(snapshot: ScannerSnapshot) -> str:
...
def compose_scanner(
    ...
    *,
    ...
    """Run the deterministic pipeline over one immutable snapshot.
    ...
    """
    if type(snapshot) is not ScannerSnapshot:
        raise CompositionInputError(
            "snapshot", "expected a ScannerSnapshot (build it via an adapter)"
        )
    if not isinstance(now, datetime) or now.tzinfo is None:
        raise CompositionInputError("now", "expected a timezone-aware datetime")
    if safety_policy is None:
        safety_policy = SafetyPolicy(policy_version=SCANNER_SAFETY_POLICY_VERSION)
    if type(safety_policy) is not SafetyPolicy:
        raise CompositionInputError("safety_policy", "expected a SafetyPolicy")
    if safety_policy.policy_version not in (
        SCANNER_SAFETY_POLICY_VERSION,
        SCANNER_LEGACY_SAFETY_POLICY_VERSION,
    ):
        raise CompositionInputError(
    ...
    return ScannerCompositionResult(
...
def _score_side(
    snapshot: ScannerSnapshot, side: str
...
def _plan_for(snapshot: ScannerSnapshot, side: str | None) -> ScenarioPlan | None:
    if side is None:
        return None
...
@dataclass(frozen=True, slots=True)
class ScannerCompositionResult:
    ...
    @classmethod
    def from_dict(cls, value: object) -> ScannerCompositionResult:
        ...
        if composition_version not in (COMPOSITION_POLICY_VERSION, COMPOSITION_POLICY_LEGACY_VERSION):
            raise CompositionInputError(
