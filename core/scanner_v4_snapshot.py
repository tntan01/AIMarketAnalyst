"""Scanner V4 snapshot persistence envelope (Bước 10; target-only).

10C — persistence schema bump.  V3 stamps integer ``persistence_schema_version =
1`` at ``controllers/scanner_controller.py:2529``; V4 consumer artifacts use
their own envelope identity ``scanner-v4-snapshot-envelope-v1`` with a
``mode``:
...
Both modes require the full V4 version identity.  A V3 payload (missing
``envelope_schema_version`` or carrying a V3 identity), a missing version, or
...
never auto-labelled V4.  The reader is a *strict* reader: it never rewrites a
V3 snapshot or upgrades it in place.

This module is not wired to runtime; the V3 controller persistence stays as-is
...
SCANNER_V4_SNAPSHOT_ENVELOPE_VERSION = "scanner-v4-snapshot-envelope-v1"
...
# V3-era scored/legacy fields that never belong in a V4 envelope.
...
class EnvelopeSideScore:
    """Side-owned score carried in the envelope (never V3 scored fields)."""
    ...
    def from_dict(cls, value: object, *, path: str = "envelope_side_score") -> EnvelopeSideScore:
        if type(value) is not dict:
            raise SnapshotEnvelopeError(
                SCANNER_V4_SCHEMA_INVALID, path, "expected an object"
            )
        expected = {
            "side",
        ...
        if side not in VALID_SIDES:
            raise SnapshotEnvelopeError(
                SCANNER_V4_SCHEMA_INVALID, f"{path}.side", "invalid side"
            )
        return cls(
            side=side,
...
@dataclass(frozen=True, slots=True)
class ScannerV4SnapshotEnvelope:
    """V4 persistence envelope (compact or full; strict; target-only)."""
...
def build_v4_snapshot_envelope(
    composition: ScannerV4CompositionResult,
    *,
    mode: str = MODE_COMPACT,
) -> ScannerV4SnapshotEnvelope:
    """Build a compact/full V4 snapshot envelope from a canonical composition.

    The envelope never re-scores and never reads V3 fields; every score is the
    ...
    return ScannerV4SnapshotEnvelope(
        envelope_schema_version=SCANNER_V4_SNAPSHOT_ENVELOPE_VERSION,
        mode=mode,
...
def snapshot_envelope_from_dict(
    ...
    """Strict reader for a persisted V4 envelope.

    Refuses unknown/missing versions, V3 identity, and any legacy scored field
    — a V3 snapshot is *audit-only*, non-replayable, and never auto-labelled V4.
    """
    if type(value) is not dict:
        raise SnapshotEnvelopeError(
    ...
    # A genuine V3 artifact is audit-only — identified before any legacy-field
    # check so it is never mislabelled as a «V4 payload with forbidden fields».
    ...
    if schema_version != SCANNER_V4_SNAPSHOT_ENVELOPE_VERSION:
    ...
    if "mode" not in value:
        raise SnapshotEnvelopeError(
            SCANNER_V4_VERSION_MISSING, f"{path}.mode", "missing mode"
        )
    mode = value["mode"]
    if mode not in VALID_ENVELOPE_MODES:
        raise SnapshotEnvelopeError(
            SCANNER_V4_VERSION_MISMATCH, f"{path}.mode", f"invalid mode {mode!r}"
        )
    ...
    if mode == MODE_FULL:
        ...
        try:
            ScannerV4CompositionResult.from_dict(composition_payload)
        except SnapshotEnvelopeError:
            raise
        except ValueError as exc:
            code = getattr(exc, "code", None) or SCANNER_V4_SCHEMA_INVALID
            raise SnapshotEnvelopeError(
...
def _is_v3_artifact(value: Mapping[str, Any]) -> bool:
    """Genuine V3 artifact detection (version/schema markers only).

    Used *before* the legacy-field check so a real V3 payload (whose V3 scored
    fields are its native shape) is classified audit-only instead of being
    mislabelled as a V4 payload carrying forbidden fields.
    """
...
def _is_v3_identity(value: Mapping[str, Any]) -> bool:
    """Cheap V3 heuristic: any V3 version marker / scored field present."""
    known_v3_values = {
        "scanner-v3",
        "scanner-features-v3",
        "smc-v2",
...
def _require_list(value: object, path: str) -> list[Any]:
    if not isinstance(value, list):
        raise SnapshotEnvelopeError(SCANNER_V4_SCHEMA_INVALID, path, "expected an array")
    return value


def _require_text(value: object, path: str, *, allow_empty: bool = False) -> str:
    if type(value) is not str:
        raise SnapshotEnvelopeError(SCANNER_V4_SCHEMA_INVALID, path, "expected a string")
    if not allow_empty and not value:
        raise SnapshotEnvelopeError(SCANNER_V4_SCHEMA_INVALID, path, "expected a non-empty string")
    return value
...
def _optional_int(value: object, path: str) -> int | None:
    ...
    if type(value) is not int:
        raise SnapshotEnvelopeError(SCANNER_V4_SCHEMA_INVALID, path, "expected int or null")
    return value
...
def _parse_datetime(value: object, path: str) -> datetime:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise SnapshotEnvelopeError(SCANNER_V4_SCHEMA_INVALID, path, "must be timezone-aware")
