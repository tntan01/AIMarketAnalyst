"""Scanner V4 strict snapshot replay (Bước 10; target-only).

10C — a V4 snapshot is replayed **strictly and deterministically**:
...
* replay runs the single V4 decision pipeline (composition validation ->
...
* a V3 snapshot, a payload with missing/mismatched versions, or a forbidden
  legacy scored field is **non-replayable**: it is refused (audit-only) and
  is never routed through the V4 decision path.
...
from core.scanner_v4_snapshot import (
    MODE_FULL,
...
SCANNER_V4_REPLAY_VERSION = "scanner-v4-replay-v1"
...
class SnapshotReplayOutcome:
    """Deterministic replay outcome over a full V4 envelope."""
...
def classify_snapshot_envelope(value: object) -> str:
    ...
    try:
        ...
        return "invalid"
    if payload.get("envelope_schema_version") == SCANNER_V4_SNAPSHOT_ENVELOPE_VERSION:
        # A V4 envelope: replayable only in full mode with an embedded composition
        # that itself passes the STRICT reader (exact nested identity — V3/mixed/
...
def replay_snapshot_envelope(
    ...
    *,
    ...
    """Strict, deterministic replay of a full V4 snapshot envelope.

    Refuses ``compact`` envelopes (display-only), V3 artifacts, missing or
    ...
    and routed through the single V4 decision path; the replayed candidate is
    then compared field-by-field against the stored envelope.  Nothing is
    rewritten.
    """
    # 1. Strict envelope read (fail-closed; V3 -> audit-only non-replayable).
    try:
    ...
    try:
        composition = ScannerV4CompositionResult.from_dict(composition_payload)
    except ValueError as exc:
        return _failed(getattr(exc, "code", None) or SCANNER_V4_SCHEMA_INVALID)

    router: RoutedCandidate = route_scanner_v4(
        composition,
...
def _setup_for(
    ...
    envelope: ScannerV4SnapshotEnvelope,
    candidate_status: str | None,
) -> int | None:
...
def _require_mapping(value: object) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise SnapshotReplayError(
            SCANNER_V4_SCHEMA_INVALID, "snapshot_payload", "expected a JSON object"
        )
    return value