"""Scanner V4 execution readiness (Bước 08; target-only, not live-wired yet).
...
and never re-interprets raw actions or gates itself.  (V3 re-derived readiness
...
_ORDER_PREPARED_CODES = frozenset({V4_ORDER_PREPARED, V4_ORDER_NOT_PREPARED})
...
class ExecutionReadiness:
    ...
    def with_candidate(
        ...
        codes = base_codes + [V4_ORDER_PREPARED if prepared else V4_ORDER_NOT_PREPARED]
...
def evaluate_execution_readiness(
    ...
    codes: list[str] = []
    if fresh:
        codes.append(V4_EXECUTION_FRESH_OK)
    if not can_execute:
        codes.append(V4_EXECUTION_NOT_READY)
    codes.append(V4_EXECUTION_REVALIDATION_REQUIRED)
