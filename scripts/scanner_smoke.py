...
* one canonical V4 snapshot → ``compose_scanner`` → row → candidate (via the
...
def _run() -> dict[str, object]:
    ...
    return {
        "smoke_version": SCANNER_RELEASE_VERSION,
...
def _assert_release_contract(report: dict[str, object]) -> None:
    assert report["threshold_policy"]["policy_version"] == "scanner-threshold-policy"
...
def _run_pathb() -> dict[str, object]:
    ...
    ``config/scanner_order_policy.json`` — proves the Bước-13 wiring while
    ...
    try:
        run_pair_from_live(d1[:30], h4, h1, "XAUUSD", _pathb_safety(),
                              now=NOW, captured_at=NOW)
...
def main() -> None:
    ...
    assert pathb["row_identity"]["feature_version"] == "scanner-features"
