...
Run:  python scripts/scanner_b11_validation.py

Runs the Bước 11 verification-test files, captures the real pass/fail counts and
the versions they lock, and writes ``reports/scanner/validation_b11.json``.
...
from ui.scanner_v4_presentation import SCANNER_PRESENTATION_SCHEMA_VERSION

OUTPUT = PROJECT_ROOT / "reports" / "scanner" / "validation_b11.json"

# The Bước 11 verification-test files (target-only; not wired into runtime).
TEST_FILES = (
    "tests/test_scanner_invariants.py",
    "tests/test_scanner_scenario_matrix.py",
    "tests/test_scanner_integration.py",
    "tests/test_scanner_oracle.py",
    "tests/test_scanner_snapshot.py",
    "tests/test_scanner_replay.py",
    "tests/test_scanner_candidate.py",
    "tests/test_scanner_ranking.py",
...
def main() -> None:
    ...
    manifest = {
        "schema": "scanner-b11-validation",
        "generated_by": "scripts/scanner_b11_validation.py",
