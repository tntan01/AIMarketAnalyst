"""Scanner V4 ranking (Bước 08, target-only).
...
from core.scanner_v4_execution_readiness import ExecutionReadiness
from core.scanner_v4_ranking import (
    DEFAULT_STATUS_ORDER,
    DEFAULT_WITHIN_GROUP_KEYS,
    RankPolicyError,
    ScannerV4RankingPolicy,
...
class TestStatusRankedFirst:
    ...
    def test_status_first_ignores_phenomenal_within_group_scores(self):
        ...
        ranked = rank_scanner_v4_candidates([
            _candidate(status=READY_NOW, setup=5, rr="1/1", proximity=0.1),
            _candidate(status=WATCH_ZONE, setup=100, rr="9/1", proximity=0.9),
        ])
...
class TestWithinGroupKeys:
    ...
    def test_repeated_ranking_is_stable(self):
        ...
        once = rank_scanner_v4_candidates(candidates)
        twice = rank_scanner_v4_candidates(candidates)
...
class TestNoNewsSpreadMacroInfluence:
    ...
    def test_macro_gate_status_never_ties_break(self):
        ...
        assert rank_scanner_v4_candidates([c2, c1]) == rank_scanner_v4_candidates([c1, c2])
...
class TestGrouped:
    ...
    def test_empty_input_gives_all_clear(self):
        assert grouped_scanner_v4_candidates([]) == {}
        assert rank_scanner_v4_candidates([]) == ()


class TestPolicyContract:
    def test_duplicate_status_rejected(self):
        with pytest.raises(RankPolicyError):
            ScannerV4RankingPolicy(status_order=(READY_NOW, READY_NOW))

    def test_empty_version_rejected(self):
        with pytest.raises(RankPolicyError):
            ScannerV4RankingPolicy(ranking_version="")
    ...
    def test_custom_status_order_is_refused_at_runtime(self):
        ...
        with pytest.raises(RankPolicyError):
            rank_scanner_v4_candidates(candidates, policy=custom)

    def test_non_default_within_group_keys_refused_at_runtime(self):
        custom = ScannerV4RankingPolicy(within_group_keys=("setup_score",))
        with pytest.raises(RankPolicyError):
            rank_scanner_v4_candidates([_candidate(status=READY_NOW)], policy=custom)

    def test_non_canonical_version_refused_at_runtime(self):
        custom = ScannerV4RankingPolicy(ranking_version="scanner-ranking-v3")
        with pytest.raises(RankPolicyError):
            rank_scanner_v4_candidates([_candidate(status=READY_NOW)], policy=custom)