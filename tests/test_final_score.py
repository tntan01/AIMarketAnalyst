"""Scanner Step 06: target-only FinalScore fallback + rounding contract.
...
import core.final_score as _FSV
...
class TestTechnicalFailClosed:
    ...
    def test_invalid_technical_has_no_fallback_at_all(self):
        # legacy copied signal into evidence; Scanner must refuse to produce a
...
class TestContractInvariants:
    ...
    def test_versions_exposed(self):
        ...
        assert result.scoring_version == SCANNER_SCORING_VERSION
...
# ---------------------------------------------------------------------------
# Fixed weights: no custom/adaptive input can influence the blend.
# ---------------------------------------------------------------------------
...
class TestWeightImmutability:
    ...
    def test_dunder_signature_is_locked(self):
        text = (_CORE_DIR / "final_score.py").read_text(encoding="utf-8")
        for forbidden in (
            "normalize_weights",
            ...
            assert forbidden not in text, f"legacy adaptive/renormalize marker {forbidden!r} leaks in"
...
# ---------------------------------------------------------------------------
# Target-only ownership: single constructor for the result, no wiring into
# the executable runtime, no dual path.
# ---------------------------------------------------------------------------


class TestOwnershipDeduplication:
    def test_only_final_score_constructs_the_result(self):
        origins: list[str] = []
        for py in _CORE_DIR.glob("*.py"):
            if py.name == "final_score.py":
                continue
            if "FinalScoreResult(" in py.read_text(encoding="utf-8"):
                origins.append(py.name)
        assert not origins, f"FinalScoreResult constructed outside final_score.py: {origins}"
    ...
    def test_final_score_not_referenced_by_runtime_modules(self, module):
        path = _CORE_DIR / module
        if path.exists():
            # The final-score module is now `core.final_score`. Assert it is not
            # imported, without flagging the unrelated legacy engine
            # (`core.final_score_engine`) or local `final_score` result keys.
            import re

            text = path.read_text(encoding="utf-8")
            assert not re.search(r"core\.final_score(?!_engine)", text)

    def test_v3_engine_kept_intact_no_dual_path(self):
        # The legacy engine is not deleted in this step; only the ledger documents
        # which of its paths Bước 07/12 must remove.  It must not reference it.
        text = (_CORE_DIR / "final_score_engine.py").read_text(encoding="utf-8")
        # The legacy engine file contains the word "final_score" by nature; assert it
        # does not reference the new *module* (import path) or module-only exports.
        assert "core.final_score" not in text
        assert "FINAL_SCORE_NEUTRAL_FALLBACK" not in text
        assert "0.65*technical_signal_score" not in text  # blend formula

    def test_no_v3_optimistic_default_equivalents_in_target(self):
        # legacy defaults signal=0 / execution=100.  Scanner must not re-invent them.
        text = (_CORE_DIR / "final_score.py").read_text(encoding="utf-8")
