...
``config/scanner_order_policy.json``. Its contract:
...
CONFIG_PATH = (
    Path(__file__).resolve().parents[1] / "config" / "scanner_order_policy.json"
)
...
class TestLiveConfigLoad:
    ...
    def test_default_path_is_repo_root_anchored(self):
        assert DEFAULT_ORDER_POLICY_FILENAME == "scanner_order_policy.json"
