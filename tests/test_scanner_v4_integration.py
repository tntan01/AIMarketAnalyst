"""Scanner V4 — target-only integration chain (Bước 11).

Drives the single V4 path end-to-end from one immutable composition through
...
Chain exercised (all target-only, nothing wired into the V3 runtime):
...
3. the **snapshot envelope** (full) round-trips through the strict reader and
   **replays** to the same candidate; compact envelopes are display-only
   (never replayable) and a V3 artifact is audit-only;
...
from core.scanner_v4_backtest_contract import verify_composition_parity
...
from core.scanner_v4_replay import (
...
class TestCompositionToRow:
    def test_row_carries_identity_scores_and_gates(self) -> None:
        ...
        assert scanner_v4_row_from_dict(row.to_dict()).to_dict() == row.to_dict()
...
class TestSnapshotAndReplay:
    def test_full_envelope_round_trips_and_replays_matching(self) -> None:
        composition = compose(build_snapshot())
        envelope = build_v4_snapshot_envelope(composition, mode=MODE_FULL)
        ...
        revalidated = ScannerV4CompositionResult.from_dict(envelope.to_dict()["composition"])
    ...
    def test_compact_envelope_is_display_only_never_replayable(self) -> None:
        composition = compose(build_snapshot())
        envelope = build_v4_snapshot_envelope(composition, mode=MODE_COMPACT)
    ...
    def test_v3_artifact_is_audit_only(self) -> None:
        ...
        assert outcome.replayable is False
        # A V3 artifact is refused for replay, never improvised into V4.
...
class TestRouterAndCandidate:
    ...
    def test_structural_reject_forbidden_legacy_keys(self) -> None:
        # A V3-shaped payload with forbidden legacy scored fields must fail
        # closed at the router, never improvised into a V4 candidate.
        payload = {
        ...
        routed = route_scanner_v4(payload, thresholds=DEFAULT_THRESHOLD_POLICY, entry_confirmation="confirmed")
...
class TestRankingChain:
    def test_status_first_order_integrated(self) -> None:
        ...
        ranked = rank_scanner_v4_candidates(candidates)
        assert [c.candidate_status for c in ranked] == [
            READY_NOW,
            WAITING_CONFIRMATION,
            BLOCKED,
        ]
        groups = grouped_scanner_v4_candidates(candidates)
...
class TestPresenterChain:
    def test_presentation_exactly_four_components_and_gate_statuses(self) -> None:
        composition = compose(build_snapshot())
        presentation = build_scanner_v4_presentation(composition)
