"""Scanner V4 live producers tests (Bước 3 — production builders).

Proves the production adapters in :mod:`core.scanner_v4_live_producers` build the
V4 inputs from live app/MT5 state without fabricating anything:

* ``resolve_technical_regime`` — parity vs V3 ``_resolve_regime_key`` over every
  primary, and that it only ever returns the VALID V4 regime vocabulary;
...
from core.scanner_v4_composition import CompositionInputError, SideSnapshot
...
from core.scanner_v4_models import SCANNER_V4_SAFETY_POLICY_VERSION
...
# ---------------------------------------------------------------------------
# resolve_technical_regime (parity vs V3 port)
# ---------------------------------------------------------------------------
...
def test_regime_mapping_parity(case, monkeypatch):
    monkeypatch.setattr(
        "core.scanner_v4_live_producers.detect_market_regime",
...
def test_derive_live_analysis_produces_valid_inputs(candles):
    ...
    raws = a["raws"]
    assert raws.features_version == "scanner-v4-features-v1"
