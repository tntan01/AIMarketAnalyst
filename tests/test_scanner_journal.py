"""Scanner journal tests (Bước 10; target-only; additive; 10D).

Proves the additive journal contract:

* the converter reads ONLY canonical side-owned scores (never
...
* the legacy journal (rows carrying legacy scored fields) is never mixed into
  the canonical chain, never re-labelled and never replayed into its shape.

Runtime isolation: the live legacy journal services are untouched.
"""
...
from services.scanner_journal_converters import (
    JOURNAL_KEYS,
    journal_row_from_dict,
    journal_row_from_composition,
)
...
class TestConverterReadsCanonicalOnly:
    def test_row_is_canonical_and_versioned(self):
        row = journal_row_from_composition(_run(), now=_NOW)
        assert row.journal_schema_version == SCANNER_JOURNAL_SCHEMA_VERSION
    ...
    def test_side_scores_come_from_canonical_side_score(self):
        composition = _run()
        row = journal_row_from_composition(composition, now=_NOW)
    ...
    def test_row_never_carries_v3_scored_fields(self):
        from services.scanner_journal_models import FORBIDDEN_V3_JOURNAL_FIELDS

        payload = journal_row_from_composition(_run(), now=_NOW).to_dict()
    ...
    def test_row_is_strict_per_version_id(self):
        ...
        assert set(row_dict) == JOURNAL_KEYS
...
class TestEvidencePartition:
    ...
    def test_forged_different_policy_identity_refused_at_reader(self):
        # §12.1 / §7.3: a journal row must carry the EXACT locked identity.
    ...
    def test_v3_row_cannot_enter_partition_logic(self):
        # A legacy-shaped row is refused at the strict reader before partitioning.
        row = journal_row_from_composition(_run(), now=_NOW)
        payload = row.to_dict()
...
class TestStrictReader:
    ...
    def test_refuses_v3_journal_row_audit_only(self):
        ...
        with pytest.raises(JournalError) as exc:
            journal_row_from_dict(payload)
        # a legacy row is audit-only (never reloadable as a canonical row)
...
class TestV3RuntimeUntouched:
    def test_v3_journal_modules_do_not_import_converter(self):
        from pathlib import Path

        project_root = Path(__file__).resolve().parents[1]
        for rel in ("services/journal_converters.py", "services/journal_models.py"):
            text = (project_root / rel).read_text(encoding="utf-8")
            assert "scanner_journal" not in text, f"{rel} must not import the journal"