"""Scanner V4 strict replay tests (Bước 10; target-only; 10C).

Proves deterministic, strict replay:

* a ``full`` envelope is the only replayable artifact; replay routes the
  embedded composition through the single V4 decision path and compares the
  replayed candidate byte-for-byte against the stored envelope fields;
* a ``compact`` envelope, a V3 snapshot and a missing/mismatched-version
  payload are **non-replayable** — refused, never routed through the V4 path,
  never rewritten;
...
from core.scanner_v4_replay import (
    SCANNER_V4_REPLAY_VERSION,
    SnapshotReplayOutcome,
    classify_snapshot_envelope,
    replay_snapshot_envelope,
)
from core.scanner_v4_snapshot import MODE_COMPACT, MODE_FULL, build_v4_snapshot_envelope
...
class TestNonReplayable:
    ...
    def test_v3_artifact_is_non_replayable(self):
        ...
        assert outcome.replayable is False
        assert outcome.match is False
        assert SCANNER_V4_LEGACY_V3_AUDIT_ONLY in outcome.reason_codes
    ...
    def test_missing_version_is_non_replayable(self):
        ...
        assert outcome.replayable is False
        assert outcome.match is False
        assert outcome.reason_codes == (SCANNER_V4_VERSION_MISSING,) or (
            SCANNER_V4_VERSION_MISSING in outcome.reason_codes
        )
    ...
    def test_mismatched_version_is_non_replayable(self):
        ...
        outcome = replay_snapshot_envelope(payload, thresholds=make_default_threshold_policy())
        assert outcome.replayable is False
        assert outcome.reason_codes and SCANNER_V4_VERSION_MISMATCH in outcome.reason_codes
...
class TestReplayVersionIdentity:
    def test_replay_version_constant_exists(self):
        assert SCANNER_V4_REPLAY_VERSION.startswith("scanner-v4-replay")
        assert isinstance(SCANNER_V4_REPLAY_VERSION, str) and SCANNER_V4_REPLAY_VERSION