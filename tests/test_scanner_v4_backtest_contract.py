"""Scanner V4 backtest parity contract (Bước 09, target-only).

Mục 9A: backtest MUST call the same ``compose_scanner_v4`` (and the same
...
from core.scanner_v4_backtest_contract import (
    SCANNER_V4_BACKTEST_CONTRACT_VERSION,
    SCANNER_V4_CANDIDATE_LEDGER_VERSION,
    SCANNER_V4_CANDIDATE_REPLAY_VERSION,
    SCANNER_V4_FROZEN_STRATEGY_VERSION,
    INCOMPATIBLE_ARTIFACT_KIND,
    V3_AUDIT_ONLY_ARTIFACT_KIND,
    V4_REPLAYABLE_ARTIFACT_KIND,
    ArtifactClassification,
    ScannerV4ArtifactError,
    classify_backtest_artifact,
    require_v4_replayable,
...
def _pair() -> tuple[ScannerV4CompositionResult, ScannerV4CompositionResult]:
...
def _candidate_pair():
    ...
    out_live = route_scanner_v4(live.to_dict(), thresholds=thresholds, entry_confirmation="confirmed")
    out_bt = route_scanner_v4(backtest.to_dict(), thresholds=thresholds, entry_confirmation="confirmed")
...
class TestCompositionParity:
    def test_identical_input_parity_passes(self):
        ...
        assert report.contract_version == SCANNER_V4_BACKTEST_CONTRACT_VERSION
    ...
    def test_split_snapshot_single_api_is_the_only_path(self):
        # The structural contract: `compose_scanner_v4` is one shared API and
    ...
    def test_payload_type_parity_covers_canonical_block(self):
        ...
        # A V4 envelope has no legacy decision block; the router would refuse it.
    ...
    def test_a_real_score_difference_flags_violation(self):
        ...
        assert report.reason_codes == (SCANNER_V4_BACKTEST_PARITY_VIOLATION,)
...
class TestNoBacktestSpecificPath:
    def test_backtest_snapshot_has_no_scorer_semantics(self):
        ...
        # snapshot model carries no V3 score fields.
...
# ---------------------------------------------------------------------------
# 9C artifact classification (read-only V3, replayable V4)
# ---------------------------------------------------------------------------


class TestArtifactClassification:
    def test_full_v4_composition_envelope_is_replayable(self):
        verdict = classify_backtest_artifact(_compose(_snapshot()).to_dict())
        assert verdict.kind == V4_REPLAYABLE_ARTIFACT_KIND
        assert verdict.reason_codes == ()

    def test_single_v4_version_field_is_not_replayable(self):
        # §12.1 / §7.3: a single version field is NOT a complete V4 artifact.
    ...
    def test_single_v4_contract_field_is_not_replayable(self):
        verdict = classify_backtest_artifact(
            {"backtest_contract_version": SCANNER_V4_BACKTEST_CONTRACT_VERSION}
        )
        assert verdict.kind == INCOMPATIBLE_ARTIFACT_KIND
        assert SCANNER_V4_VERSION_MISSING in verdict.reason_codes
    ...
    def test_non_dict_incompatible(self):
        ...
        assert verdict.kind == INCOMPATIBLE_ARTIFACT_KIND
        assert SCANNER_V4_SCHEMA_INVALID in verdict.reason_codes

    def test_require_v4_replayable_returns_unchanged(self):
        value = _compose(_snapshot()).to_dict()
        assert require_v4_replayable(value) == value

    def test_require_v4_replayable_refuses_v3(self):
        with pytest.raises(ScannerV4ArtifactError):
            require_v4_replayable(
                {"candidate_ledger_version": "backtest-candidate-ledger-v1"}
            )

    def test_v4_versions_are_distinct_from_v3(self):
        assert SCANNER_V4_BACKTEST_CONTRACT_VERSION != "phase0-backtest-safety-v1"
        assert SCANNER_V4_CANDIDATE_LEDGER_VERSION != "backtest-candidate-ledger-v1"
        assert SCANNER_V4_CANDIDATE_REPLAY_VERSION != "candidate-replay-v1"
        assert SCANNER_V4_FROZEN_STRATEGY_VERSION != "frozen-strategy-config-v1"
