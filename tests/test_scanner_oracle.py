"""Scanner — oracle fixture + static legacy-isolation checks (Bước 11).
...
A. **Oracle**: a frozen, independent reference for the locked Mục 4.3 technical
   ...
   so any drift in the scorer or the final blend is caught.  A versioned
   fixture must carry the locked version identities.

B. **Static legacy isolation**: the target-only modules must not reference the
   legacy six-component scored fields (``risk_condition`` / ``macro_alignment`` /
   ``opportunity_score`` / ``scanner_action`` / ``scanner_group`` /
   ``expected_effective_rr`` / ``best_score``), must not run the legacy runtime
...
from core.final_score import score_final_score
...
# target-only source files scanned by the static isolation checks.
SOURCE_FILES = (
    "core/technical_signal_scorer.py",
    "core/final_score.py",
    "core/market_safety_gate.py",
    "core/macro_gate.py",
    "core/scanner_composition.py",
    "core/scanner_candidate.py",
    "core/scanner_threshold_policy.py",
    "core/scanner_execution_readiness.py",
    "core/scanner_ranking.py",
    "core/scanner_v4_strategy_router.py",
    "core/scanner_row.py",
    "core/scanner_snapshot.py",
    "core/scanner_replay.py",
    "core/scanner_v4_observability.py",
    "core/scanner_v4_session_review.py",
    "core/scanner_backtest_contract.py",
    "core/scanner_candidate_ledger.py",
    "core/scanner_calibration.py",
    "core/scanner_safety_audit.py",
    "core/scanner_config_invalidation.py",
    "services/scanner_journal_models.py",
    "services/scanner_journal_converters.py",
    "ui/scanner_v4_presentation.py",
)

# legacy six-component scored fields that must never be read as scored evidence.
...
def _collect_keys(value, out: set[str]) -> None:
    """Recursively collect every mapping key from a serialized payload."""
...
class TestCompositionOracle:
    def test_frozen_composition_carries_locked_versions_and_scores(self) -> None:
        ...
        assert buy.final_score == buy.setup_score  # alias, never a separate number
        assert result.canonical.scoring_version == SCANNER_SCORING_VERSION
...
class TestVersionedFixture:
    def test_versions_are_stamped(self) -> None:
        ...
        from core.scanner_snapshot import SCANNER_SNAPSHOT_ENVELOPE_VERSION
        ...
        assert SCANNER_SNAPSHOT_ENVELOPE_VERSION == "scanner-snapshot-envelope"


# ---------------------------------------------------------------------------
# B. Static legacy-isolation checks
# ---------------------------------------------------------------------------


class TestStaticV3Isolation:
    def test_serialized_outputs_never_emit_v3_scored_keys(self) -> None:
        # The legacy six-component scored fields must never appear as an output
        # key anywhere in a serialized artifact (row / envelope / candidate /
        ...
        from core.scanner_snapshot import MODE_FULL, build_snapshot_envelope
        from ui.scanner_v4_presentation import build_scanner_presentation

        composition = compose(build_snapshot())
        artifacts = [
            scanner_row_from_composition(composition).to_dict(),
            build_snapshot_envelope(composition, mode=MODE_FULL).to_dict(),
            build_candidate(
            ...
            build_scanner_presentation(composition).to_dict(),
            build_scanner_presentation(composition).to_dict()["side_scores"][0],
        ]
        ...
        assert not leaking, f"outputs emit legacy scored keys: {leaking}"

    def test_sources_never_import_v3_runtime_scorer(self) -> None:
        # Scanner is pure and target-only: it never runs the legacy service / controller /
        # detail screen scorer.  ``core.scanner`` (legacy) must not be imported; the
        # ``core.scanner_v4_*`` modules must not be caught by the boundary.
        # ``from core.scanner import`` matches "core.scanner\\n... import"; the
        # regex below excludes the ``_v4`` suffix so target self-imports are allowed.
        import re
        ...
        assert not offender, f"sources import the legacy runtime: {offender}"
    ...
    def test_safety_gate_only_usable_on_valid_availability(self) -> None:
        ...
        from tests.scanner_testkit import CAPTURED, safety_context
