...
breakdown/gate reason/identity, và không đọc bất kỳ field legacy nào
...
from core.scanner_candidate_ledger import (
    LedgerSideScore,
    ScannerLedgerRow,
    build_scanner_ledger_row,
)
from core.scanner_composition import ScannerCompositionResult
from core.scanner_v4_strategy_router import ROUTE_ROUTED, route_scanner
from core.scanner_threshold_policy import make_default_threshold_policy
from tests.test_scanner_composition import _run, _snapshot, _compose

NOW = datetime(2026, 8, 13, 12, 0, 0, tzinfo=timezone.utc)


def _composition_and_candidate() -> tuple[ScannerCompositionResult, ScannerV4CandidateDecision]:
...
class TestLedgerRowBuild:
    def test_row_is_side_owned_and_versioned(self):
        ...
        row = build_scanner_ledger_row(composition, candidate)
        assert isinstance(row, ScannerLedgerRow)
        assert row.candidate_ledger_version == SCANNER_CANDIDATE_LEDGER_VERSION
    ...
    def test_mismatched_artifacts_refused(self):
        ...
        with pytest.raises(ValueError):
            build_scanner_ledger_row(composition, other_out.candidate)

    def test_type_checks(self):
        composition, candidate = _composition_and_candidate()
        with pytest.raises(TypeError):
            build_scanner_ledger_row(object(), candidate)  # type: ignore[arg-type]
        with pytest.raises(TypeError):
            build_scanner_ledger_row(composition, object())  # type: ignore[arg-type]

    def test_row_to_dict_has_no_legacy_fields(self):
        composition, candidate = _composition_and_candidate()
        payload = build_scanner_ledger_row(composition, candidate).to_dict()
...
class TestLedgerToDict:
    def test_round_trip_identity(self):
        ...
        row = build_scanner_ledger_row(composition, candidate)
        payload = row.to_dict()
        assert payload["candidate_ledger_version"] == SCANNER_CANDIDATE_LEDGER_VERSION
...
class TestLedgerStandalone:
    def test_scanner_backtest_composition_yields_ledger_row(self):
        ...
        # the same ledger row.
        composition, candidate = _composition_and_candidate()
        row_a = build_scanner_ledger_row(composition, candidate)
        composition_b, candidate_b = _composition_and_candidate()
        row_b = build_scanner_ledger_row(composition_b, candidate_b)
