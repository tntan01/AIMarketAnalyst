"""Scanner V4 → UI row adapter tests (Bước 12 C2a; target-only).

Proves the single V4→UI row mapping:

* every key with a real V4 source equals its exact V4 source
...
* V3-only keys the UI still reads are the documented neutral (never fabricated);
* a V3/mixed/unknown version is REFUSED before mapping (AdapterContractError);
...
from core.scanner_v4_release import run_v4_pair, run_v4_pair_from_live
...
from tests.scanner_v4_testkit import NOW as TESTKIT_NOW
...
class TestIdentityAndMapping:
    def test_exact_identity_block(self):
        ...
        assert row["row_version"] == "scanner-v4-row-v1"
    ...
    def test_mapped_keys_equal_v4_sources(self):
        pair = _pair()
    ...
    def test_analysis_status_is_ok_not_structural_reject(self):
        # The V4 model has no structural-reject/OUT_OF_STRATEGY classification.
...
class TestV3OnlyNeutrals:
    def test_v3_only_keys_are_documented_neutral(self):
        ...
        assert row["price_vs_zone"] is None
        assert row["m15_quality"] is None

    def test_no_optimistic_scored_weapons(self):
        # Never emit a number the UI could read as a real score for V3-only keys.
        row = pair_to_ui_row(_pair())
        for key in (
...
class TestVersionRefusal:
    def test_refuses_non_v4_pair(self):
