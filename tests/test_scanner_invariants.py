"""Scanner — Mục 13 invariant / property tests (Bước 11; target-only).
...
class TestMissingSafetyNeverPass:
    ...
    def test_missing_safety_provider_error_is_not_pass(self) -> None:
        ...
        from tests.scanner_testkit import CAPTURED, safety_context, safety_policy
...
class TestLiveBacktestSameVersion:
    def test_same_identity_versions(self) -> None:
        ...
        for attr in ("scoring_version", "feature_version", "snapshot_version"):
            ...
            assert lv == bv == (SCANNER_SCORING_VERSION if attr == "scoring_version" else SCANNER_V4_FEATURE_VERSION if attr == "feature_version" else "scanner-pair-snapshot")
