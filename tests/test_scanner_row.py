"""Scanner row/API consumer tests (Bước 10; target-only; 10A).

Proves the canonical-only row contract:
...
* ``blocked_scanner_row_v4`` is a full-schema ``DATA_UNAVAILABLE`` row with
  **null** technical/setup and Safety/Macro ``UNKNOWN`` — never the legacy
  «macro 15 / zeroed buy/sell» shape;
* the persisted-row reader is strict: legacy scored fields, missing or mismatched
  row version are refused (no downgrade into a canonical row).

Runtime isolation: this test also asserts the legacy scanner runtime never imports
the row module (no partial activation before cutover).
"""
...
from tests.test_scanner_composition import _compose, _run, _snapshot
...
class TestRowFromComposition:
    def test_canonical_only_when_default_composition(self):
        row = scanner_row_from_composition(_run())
        assert row.row_version == SCANNER_ROW_VERSION
    ...
    def test_typed_input_required(self):
        with pytest.raises(TypeError):
            scanner_row_from_composition({"not": "a composition"})
...
class TestStrictReader:
    def test_round_trip(self):
        row = scanner_row_from_composition(_run())
        restored = scanner_row_from_dict(row.to_dict())
        ...
        assert restored.row_version == SCANNER_ROW_VERSION
    ...
    def test_refuses_v3_scored_fields(self):
        payload = scanner_row_from_composition(_run()).to_dict()
    ...
    def test_refuses_non_dict_payload(self):
        with pytest.raises(RowContractError) as exc:
            scanner_row_from_dict(["not", "a", "dict"])
...
class TestRuntimeIsolation10A:
    """The row consumer must never leak into the legacy executable runtime."""

    def test_v3_runtime_files_do_not_reference_consumers(self):
        ...
        for rel in v3_runtime:
            ...
            # The row *module* is now `scanner_row`, but legacy runtime symbols like
            # scanner_row_from_analysis / enrich_scanner_row also match that bare substring.
            # Assert on the consumer exports instead so legacy row helpers are not flagged.
            for token in (
                "scanner_row_from_composition",
                "scanner_row_from_dict",
                "blocked_scanner_row_v4",
            ):
                assert token not in text, f"{rel} must not import the row ({token})"
            assert "scanner_v4_presentation" not in text, f"{rel} must not import the presentation module"
            assert "ScannerSnapshotEnvelope" not in text, f"{rel} must not import the snapshot"