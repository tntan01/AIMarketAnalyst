"""Scanner V4 feature derivation tests (Path B, Bước 2+4).

Proves the candle→raw port in :mod:`core.scanner_v4_features`:

* **parity** — on a frozen fixture, the V4-ported raw formulas equal V3
  ``signal_engine.trend_alignment_score / momentum_alignment_score /
  location_quality_score`` for both sides (this is the V3-deletion gate; the
  test is retired when V3 scoring is removed);
...
# ---------------------------------------------------------------------------
# Parity vs V3 signal_engine — FROZEN SNAPSHOT (deletion gate)
# ---------------------------------------------------------------------------
# The V3 raws (trend/momentum/location) were ported to scanner_v4_features. On
# this DETERMINISTIC frozen fixture the V3 signal_engine produced exactly the
# values below (captured pre-deletion). We now assert the V4 port against the
# frozen snapshot instead of importing the deleted V3 module.
...
def _real_canonical_smc(d1, h4, h1):
    ...
    ``smc-v2`` source the V4 contract mandates.  Feeding its real output into
...
def test_smc_projected_with_canonical(candles):
    ...
    # Outermost raw the V4 scorer consumes must already be ≤15.
    for side in ("buy", "sell"):
