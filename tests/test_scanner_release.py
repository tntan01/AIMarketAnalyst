"""Scanner release wiring tests (Bước 12; target-only cutover entry).

Proves the SINGLE release wiring (``core.scanner_release``):

* ``compose_scanner`` is the only scoring/decision entry — the release
  module exposes it and never reads legacy scored fields;
* the DEFAULT threshold policy (technical 40 / setup 35 / gap 5 / R:R 2/1,
  ``scanner-threshold-policy``) is what the candidate decision layer uses —
  a single-owner default, not a fabricated/legacy-copied threshold; ``None`` still
  fails closed;
...
* end-to-end shape: composition → row → candidate → setup filter → locked rank
  → group, with exact version identity end-to-end;
* the routed candidate is intent-only (``sends_real_order=False``) — the
  release wiring never dispatches;
* legacy/mixed/unknown identity is rejected before any decision.
"""
...
def _pair(entry_confirmation: str = "confirmed"):
    return run_pair(_snapshot(), now=NOW, entry_confirmation=entry_confirmation)
...
class TestSingleEntry:
    def test_run_pair_builds_full_release_pair(self):
        ...
        assert pair.row.scoring_version == "scanner"
        assert pair.row.row_version == SCANNER_ROW_VERSION
    ...
    def test_default_threshold_policy_is_the_locked_default(self):
        ...
        # fabricated nor copied from legacy.  The release binds exactly this policy.
        assert DEFAULT_THRESHOLD_POLICY == make_default_threshold_policy()
        assert DEFAULT_THRESHOLD_POLICY.policy_version == "scanner-threshold-policy"
    ...
    def test_release_version_is_locked(self):
        assert SCANNER_RELEASE_VERSION == "scanner-release"
...
class TestSetupFilter:
    ...
    def test_filter_refuses_v3_scored_input_through_row_mapping(self):
        # ready_pairs_above_setup builds clean row dicts; a forged legacy field on
...
class TestV3Rejected:
    def test_release_refuses_v3_copied_threshold_policy(self):
        ...
        # construct a legacy/mixed policy.  Nothing legacy-copied can enter the release
    ...
    def test_release_row_reader_rejects_v3_identity(self):
        ...
        from core.scanner_row import RowContractError, scanner_row_from_dict
...
class TestLiveWiringFromCandles:
    ...
    def test_live_pair_carries_exact_identity(self):
        pair = _live_pair()
        assert pair.row.scoring_version == "scanner"
        assert pair.row.feature_version == "scanner-features"
        assert pair.row.composition_version == "scanner-composition"
    ...
    def test_live_pair_deterministic(self):
        ...
        d1, h4, h1 = _live_candles()
        a = run_pair_from_live(d1, h4, h1, "XAUUSD", _live_safety(), now=NOW,
    ...
    def test_live_pair_fails_closed_on_insufficient_history(self):
        ...
        with pytest.raises(TechnicalRawDerivationError):
            run_pair_from_live(d1[:30], h4, h1, "XAUUSD", _live_safety(),
                                  now=NOW, captured_at=NOW)
...
class TestLiveScenarioPlanWiring:
    ...
    def test_plan_flows_into_snapshot_from_real_structure(self):
        ...
        assert plan.source in ("smc_canonical_zone", "technical_zone")
        # The gate's exact R:R is the plan's own ratio (never approximated).
        from core.scanner_composition import compute_scenario_rr
    ...
    def test_plan_missing_code_disappears_when_structure_present(self):
        ...
        # The legacy-aligned zone-anchored construction (entry at the protective
