"""Scanner backtest safety-data audit (Bước 09, target-only).
...
from core.reason_codes import (
    SCANNER_SAFETY_AUDIT_MISSING,
    SCANNER_SAFETY_AUDIT_NON_PIT,
    SCANNER_SAFETY_AUDIT_UNKNOWN,
)
...
class TestEmptyInventory:
    def test_no_declared_sources_all_missing(self):
        ...
        assert report.audit_version == SCANNER_SAFETY_AUDIT_VERSION
        ...
        for category in SAFETY_AUDIT_CATEGORIES:
            ...
            assert SCANNER_SAFETY_AUDIT_MISSING in item.reason_codes, category
...
class TestNonPitIsNeverAvailable:
    def test_non_pit_source_is_never_available(self):
        ...
        assert SCANNER_SAFETY_AUDIT_NON_PIT in item.reason_codes
        assert is_eligible_for_entry(item) is False
...
class TestUnknownAndProvenance:
    ...
    def test_report_to_dict_shape(self):
        ...
        assert payload["audit_version"] == SCANNER_SAFETY_AUDIT_VERSION
...
class TestCurrentRepoState:
    def test_current_repo_has_no_pit_safety_dataset(self):
        ...
        assert SCANNER_SAFETY_AUDIT_MISSING in codes
