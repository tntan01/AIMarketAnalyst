"""Scanner V4 persistence envelope tests (Bước 10; target-only; 10C).

Proves the compact/full V4 snapshot envelope:
...
* a V3 artifact (or any missing/mismatched version) is refused — never
  re-labelled as a V4 envelope; a V3 payload is audit-only, non-replayable.

Runtime isolation: ``controllers/scanner_controller.py`` keeps its V3
``persistence_schema_version = 1`` shape and is untouched by the V4 envelope.
"""
...
class TestCompactEnvelope:
    def test_compact_round_trip_identity_preserved(self):
        env = build_v4_snapshot_envelope(_run(), mode=MODE_COMPACT)
        assert env.mode == MODE_COMPACT
        payload = env.to_dict()
        restored = snapshot_envelope_from_dict(payload)
        assert restored.to_dict() == payload  # byte-for-byte
        assert restored.envelope_schema_version == SCANNER_V4_SNAPSHOT_ENVELOPE_VERSION
    ...
    def test_full_embedding_is_strict_round_trip(self):
        composition = _run()
        full = build_v4_snapshot_envelope(composition, mode=MODE_FULL)
    ...
    def test_invalid_mode_refused(self):
        with pytest.raises(ValueError):
            build_v4_snapshot_envelope(_run(), mode="ultra")

    def test_full_to_dict_without_composition_raises(self):
        env = build_v4_snapshot_envelope(_run(), mode=MODE_FULL)
...
class TestStrictReaderContract:
    def test_refuses_v3_scored_fields(self):
        payload = build_v4_snapshot_envelope(_run(), mode=MODE_FULL).to_dict()
    ...
    def test_refuses_v3_persistence_version_field(self):
        payload = build_v4_snapshot_envelope(_run(), mode=MODE_COMPACT).to_dict()
    ...
    def test_refuses_v3_artifact_audit_only(self):
        ...
        assert exc.value.code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY
...
class TestV3RuntimeUntouched:
    def test_v3_controller_schema_still_unchanged(self):
        ...
        assert "scanner_v4_snapshot" not in controller
        assert "persistence_schema_version" in controller  # V3 shape still present