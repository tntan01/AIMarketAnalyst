"""Tests for the Scanner analysis pipeline and direct composition (Bước 07).

Target-only discipline: this module validates the composition module in
isolation.  It never wires into runtime, never checks numeric values that
...
* full-schema fail-closed — stale/future/invalid-technical snapshots produce
  DATA_UNAVAILABLE with the complete schema and no fake scores;
...
* runtime isolation — the composition module imports no legacy engine and no
...
def _selection_for(
    ...
    *,
    ...
    """Final canonical selection matching the fixture's legacy subtotal.

    Task 101-107: the Scanner technical consumer reads the FINAL selection, so
    the fixture must carry one.  ``quality_raw`` equals the subtotal the legacy
    fixture used, which is what keeps the downstream score expectations valid.
    """

    b, q, l, c = _bqlc_for(float(quality_raw))
    total = 4.0 * b + 7.0 * q + 2.0 * l + 2.0 * c
    raw = round_half_up(total)
...
def _run(source: str = "live") -> ScannerCompositionResult:
    return _compose(_snapshot(source=source))


def _code_text() -> str:
    text = (_CORE_DIR / "scanner_composition.py").read_text(encoding="utf-8")
...
class TestContract:
    def test_result_wraps_canonical_artifact(self):
        result = _run()
        assert type(result) is ScannerCompositionResult
    ...
    def test_versions_stamped(self):
        ...
        assert result.canonical.snapshot_version == "scanner-pair-snapshot"
        assert result.canonical.safety_policy_version == SCANNER_SAFETY_POLICY_VERSION
        assert result.canonical.macro_policy_version == SCANNER_MACRO_POLICY_VERSION

    def test_composition_version_constant(self):
        assert COMPOSITION_POLICY_VERSION == "scanner-composition"
        result = _run()
    ...
    def test_compose_rejects_non_snapshot(self):
        with pytest.raises(CompositionInputError):
            compose_scanner(object(), now=NOW)

    def test_compose_rejects_tz_naive_now(self):
        with pytest.raises(CompositionInputError):
            compose_scanner(_snapshot(), now=datetime(2026, 8, 13, 12, 0))

    def test_compose_rejects_wrong_policy_option_types(self):
        snapshot = _snapshot()
        with pytest.raises(CompositionInputError):
            compose_scanner(snapshot, now=NOW, safety_policy=object())
        with pytest.raises(CompositionInputError):
            compose_scanner(snapshot, now=NOW, macro_policy=object())
        with pytest.raises(CompositionInputError):
            compose_scanner(snapshot, now=NOW, options=object())
    ...
    def test_compose_scanner_is_the_single_api(self):
        sig = inspect.signature(compose_scanner)
...
class TestBlockKeepsScoreAndScenario:
    ...
    def _assert_blocked_keeps_evidence(
        self, result: ScannerCompositionResult
    ) -> None:
...
class TestScoreInvariance:
    def _score_signature(self, result: ScannerCompositionResult) -> tuple:
        return (
...
class TestDecisionMapping:
    def test_default_policies_fail_closed_to_blocked(self):
        snapshot = _snapshot()
        result = compose_scanner(snapshot, now=NOW, options=_options())
        assert result.decision.candidate_status == BLOCKED
...
class TestIsolationAndOwnership:
    def test_module_imports_modules_only(self):
        module = importlib.util.find_spec("core.scanner_composition")
        ...
        for name in (
            ...
            assert not re.search(rf"\b(?:from|import)\s+{re.escape(name)}\b", text), (
                f"legacy module {name!r} imported by composition"
            )
            assert not re.search(rf"\bfrom\s+core\.{re.escape(name)}\b", text), (
                f"legacy core.{name} imported by composition"
            )
    ...
    def test_no_forbidden_pipeline_markers(self):
        ...
        for marker in (
            ...
            assert marker not in text, f"legacy marker {marker!r} leaks into composition"
    ...
    def test_composition_constructors_are_single_owned(self):
        ...
        # seam (core/scanner_order_policy.py) materializes the ComposeOptions
        ...
        OPTIONS_OWNER = {"scanner_order_policy.py"}
        origins: list[str] = []
        for py in _CORE_DIR.glob("*.py"):
            if py.name == "scanner_composition.py":
                continue
            content = py.read_text(encoding="utf-8")
            for cursor in ("ScannerCompositionResult(", "ScannerSnapshot(", "ComposeOptions("):
    ...
    def test_runtime_modules_do_not_reference_composition(self):
        for module in (
            ...
            if path.exists():
                assert (
                    "scanner_composition" not in path.read_text(encoding="utf-8")
                ), f"{module} must stay legacy until atomic cutover"
