"""Scanner live gate producers — controller-side unit tests.

Covers the REAL-data producers wired into the scan-level states
...
class TestConsecutiveLosses:
    """Legacy account-guard harmonization (core/account_guard.py convention)."""

    def test_counts_trailing_losses(self):
        trades = [_trade(0.2, -1.0), _trade(0.5, -0.5), _trade(1, 0.8)]
        assert _consecutive_losses(trades) == 2
    ...
    def test_missing_result_breaks_streak_like_breakeven(self):
        ...
        assert _consecutive_losses(trades) == 1
...
class TestFetchPacketAccountState:
    def test_packet_carries_per_symbol_account_state(self):
        from core.scanner_composition import AccountState
