"""Step-02 tests for the non-runtime Scanner V4 domain contract."""
...
def _assert_dict_and_json_invalid(
    payload: object,
    expected_code: str = SCANNER_V4_SCHEMA_INVALID,
) -> None:
    ...
    with pytest.raises(ScannerV4ContractError) as dict_error:
        deserialize_canonical_pair_snapshot(payload)
    with pytest.raises(ScannerV4ContractError) as model_dict_error:
        CanonicalPairSnapshot.from_dict(payload)
    with pytest.raises(ScannerV4ContractError) as json_error:
...
def test_target_versions_are_locked_without_activating_runtime_v3():
    ...
    assert dict(SCANNER_V4_VERSION_FIELDS) == {
        ...
        "ranking_version": "scanner-ranking-v4",
        "snapshot_version": "scanner-pair-snapshot-v4",
    }


def test_all_v4_models_round_trip_without_data_loss():
    snapshot = _snapshot()
...
def test_creator_stamps_exact_versions_and_payload_has_exact_canonical_blocks():
    ...
    assert set(payload) == set(SCANNER_V4_VERSION_FIELDS) | {
        "snapshot_id",
        "symbol",
        "captured_at",
        "side_scores",
    ...
    assert classify_scanner_payload(payload).kind == PAYLOAD_V4


@pytest.mark.parametrize("field", tuple(SCANNER_V4_VERSION_FIELDS))
def test_missing_version_or_schema_fails_closed(field: str):
    ...
    assert classification.reason_codes == (SCANNER_V4_VERSION_MISSING,)
    with pytest.raises(ScannerV4ContractError) as exc_info:
        deserialize_canonical_pair_snapshot(payload)
    assert exc_info.value.code == SCANNER_V4_VERSION_MISSING
...
def test_mismatched_version_or_schema_fails_closed(field: str):
    ...
    assert classification.kind == PAYLOAD_INVALID
    assert classification.replayable is False
    assert classification.reason_codes == (SCANNER_V4_VERSION_MISMATCH,)
    with pytest.raises(ScannerV4ContractError) as exc_info:
        deserialize_canonical_pair_snapshot(payload)
    assert exc_info.value.code == SCANNER_V4_VERSION_MISMATCH
...
def test_blank_wrong_type_and_unknown_fields_fail_closed_without_coercion():
    ...
    assert exc_info.value.code == SCANNER_V4_SCHEMA_INVALID
...
def test_forbidden_scored_fields_are_rejected_at_any_depth(forbidden: str):
    ...
    assert classification.replayable is False
    assert classification.reason_codes == (SCANNER_V4_FORBIDDEN_SCORED_FIELD,)
    with pytest.raises(ScannerV4ContractError) as exc_info:
        deserialize_canonical_pair_snapshot(payload)
    assert exc_info.value.code == SCANNER_V4_FORBIDDEN_SCORED_FIELD
    with pytest.raises(ScannerV4ContractError) as json_error:
        CanonicalPairSnapshot.from_json(json.dumps(payload))
    assert json_error.value.code == SCANNER_V4_FORBIDDEN_SCORED_FIELD
...
def test_explicit_v3_is_legacy_audit_only_non_replayable_and_not_relabelled():
    ...
    assert classification.replayable is False
    assert SCANNER_V4_LEGACY_V3_AUDIT_ONLY in classification.reason_codes
    serialized_classification = classification.to_dict()
    assert serialized_classification["observed_versions"]["scoring_version"] == (
        "scanner-v3"
    )
    assert SCANNER_V4_SCORING_VERSION not in str(serialized_classification)
    assert SCANNER_V4_FEATURE_VERSION not in str(serialized_classification)
    with pytest.raises(ScannerV4ContractError) as exc_info:
        deserialize_canonical_pair_snapshot(artifact)
    assert exc_info.value.code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY


def test_unversioned_payload_is_invalid_and_never_gets_v4_defaults():
    ...
    assert classification.replayable is False
    assert classification.reason_codes == (SCANNER_V4_VERSION_MISSING,)
    assert SCANNER_V4_SCORING_VERSION not in str(classification.to_dict())
...
def test_score_types_and_final_alias_are_strict():
    ...
    with pytest.raises(ScannerV4ContractError):
...
def test_strict_json_rejects_duplicate_keys_and_non_finite_numbers():
    ...
    with pytest.raises(ScannerV4ContractError) as duplicate_error:
        CanonicalPairSnapshot.from_json(duplicate)
    assert duplicate_error.value.code == SCANNER_V4_SCHEMA_INVALID

    non_finite = snapshot.to_json().replace('"confidence":0.8', '"confidence":NaN')
    with pytest.raises(ScannerV4ContractError) as finite_error:
        CanonicalPairSnapshot.from_json(non_finite)
    assert finite_error.value.code == SCANNER_V4_SCHEMA_INVALID


def test_auxiliary_target_constants_match_payload_fields():
    payload = _snapshot().to_dict()
    assert payload["output_schema_version"] == SCANNER_V4_OUTPUT_SCHEMA_VERSION
    assert payload["safety_policy_version"] == SCANNER_V4_SAFETY_POLICY_VERSION
    assert payload["macro_policy_version"] == SCANNER_V4_MACRO_POLICY_VERSION
    assert payload["ranking_version"] == SCANNER_V4_RANKING_VERSION
    assert payload["snapshot_version"] == SCANNER_V4_SNAPSHOT_VERSION
...
def test_mixed_v3_v4_identity_is_invalid_for_dict_and_json(mutate: str):
    payload = _snapshot().to_dict()
    ...
    _assert_dict_and_json_invalid(payload, SCANNER_V4_VERSION_MISMATCH)


def test_malformed_v4_scoring_marker_with_v3_shape_is_invalid_not_legacy():
    payload = {
    ...
    _assert_dict_and_json_invalid(payload, SCANNER_V4_VERSION_MISSING)


def test_snapshot_provenance_cannot_duplicate_contract_identity_even_when_equal():
    payload = _snapshot().to_dict()
    payload["provenance"]["scoring_version"] = SCANNER_V4_SCORING_VERSION
...
def test_all_v4_provenance_blocks_reject_duplicate_or_conflicting_identity(
    provenance_path: str,
    identity_field: str,
...
def test_pure_v3_classification_and_reader_error_are_consistent_for_dict_and_json():
    ...
    with pytest.raises(ScannerV4ContractError) as json_error:
        CanonicalPairSnapshot.from_json(encoded)
    assert dict_error.value.code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY
    assert model_dict_error.value.code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY
    assert json_error.value.code == SCANNER_V4_LEGACY_V3_AUDIT_ONLY


def test_runtime_v3_side_scores_and_ranking_keys_remain_legacy_not_v4_intent():
    artifact = {
        "scorer_version": "scanner-v3",
        "feature_version": "scanner-features-v3",
...
def test_non_object_legacy_signature_is_invalid_not_legacy():
    ...
    assert classification.kind == PAYLOAD_INVALID
    assert classification.reason_codes == (SCANNER_V4_SCHEMA_INVALID,)
    with pytest.raises(ScannerV4ContractError) as exc_info:
        CanonicalPairSnapshot.from_json(encoded)
    assert exc_info.value.code == SCANNER_V4_SCHEMA_INVALID
