...
class FakeQuoteRateProvider:
    def __init__(self, *, rate: float | None = 1.25, raises: bool = False) -> None:
        self.rate = rate
        self.raises = raises
        self.calls: list[str] = []

    def quote_to_usd_rate(self, quote_currency: str) -> float | None:
        self.calls.append(quote_currency)
        if self.raises:
            raise RuntimeError("rate provider failure")
        return self.rate


def test_resolve_quote_to_usd_rate_uses_injected_provider():
    provider = FakeQuoteRateProvider(rate=1.25)

    rate = _resolve_quote_to_usd_rate("GBP/EUR", provider=provider)

    assert rate == 1.25
    assert provider.calls == ["EUR"]


def test_resolve_quote_to_usd_rate_without_provider_does_not_open_mt5():
    assert _resolve_quote_to_usd_rate("GBP/EUR") is None


def test_resolve_quote_to_usd_rate_handles_provider_errors():
    provider = FakeQuoteRateProvider(raises=True)

    assert _resolve_quote_to_usd_rate("GBP/EUR", provider=provider) is None
