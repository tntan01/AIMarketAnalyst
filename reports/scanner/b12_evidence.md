...
`docs/scanner/scanner-architecture.md` §12. Rollback tag `v3-runtime-pre-cutover`
...
> `core/scanner_v4_order_policy.py` / `config/scanner_order_policy.json`). Con số
> 3708 dưới đây là state của riêng mốc cutover này. Xem §13.1 trong
> `docs/scanner/scanner-architecture.md`.
...
$ python -m pytest \
    ...
    tests/test_scanner_features.py -q
24 passed
```
...
The V3 raw formulas were ported verbatim to `core/scanner_features.py` (documented
...
[tests/test_scanner_features.py](tests/test_scanner_features.py), 19 passed):
...
signal_engine                -> core/analysis_pipeline.py,core/risk_engine.py
                                (the core/scanner_features.py hit is a DOCSTRING port note, not an import)
...
`scripts/scanner_smoke.py` (extended) → `reports/scanner-v4/release_b12_smoke.json`
...
- **journal**: `services/scanner_journal_models.py` →
  ...
  [tests/test_scanner_journal.py](tests/test_scanner_journal.py)::test_row_is_canonical_and_versioned).
...
[core/scanner_backtest_contract.py](core/scanner_backtest_contract.py)
...
[core/scanner_candidate.py](core/scanner_candidate.py) `__post_init__` rejects any
non-`False` value; asserted in [tests/test_scanner_candidate.py](tests/test_scanner_candidate.py)
