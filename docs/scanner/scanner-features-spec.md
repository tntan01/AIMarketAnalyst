...
- **Tái sử dụng** `core/technical_context.py:build_technical_snapshot` và
  ...
  trong danh sách xóa (cùng loại với `smc_models`). `signal_engine`.
  `*_alignment_score` là thứ cần port.
- **Port công thức** của 3 raw từ `signal_engine` sang module mới (bản copy có kiểm
  chứng bằng parity test Bước 4), để sau khi xóa `signal_engine` đường live vẫn chạy.
...
> Chỉ `confluence_count>=3 → +5` từng kích hoạt. Port giữ NGUYÊN công thức (kể cả
...
Đây là mâu thuẫn contract: contract đã khóa không cho phép thay đổi `smc-v2`, nên
không thể "bịa" một producer mới sinh ra chuỗi `smc-v2` từ một module mới mà không
...
- **(a) Giữ `smc_context` + `smc_scorer` như producer canonical-SMC retained** (cùng
  ...
  contract mà Scanner đòi, không phải "đường executable scoring". Mâu thuẫn với danh
  sách xóa nhưng tôn trọng contract khóa.
- **(b) Port toàn bộ `score_smc`/`build_smc_context` sang module mới** và tạo
  `SmcScoringResult` kèm `scoring_version="smc-v2"` — nhưng chuỗi version vẫn là chuỗi
  "chuỗi version vẫn là chuỗi legacy", tạo identity khó giải thích; chi phí port 949+1223 dòng, có nhánh AI-zone-
  auditor/backtest-cache bị cắt.
- **(c) Chỉ port 3 raw đầu (trend/momentum/location) trong Path B**, và sau này (Bước
  12 C2/E) chuyển sang 1 producer SMC chính thống của Scanner riêng — nhưng hiện tại Scanner
...
"scoring executable" mà là producer contract mà Scanner đã khóa; phù hợp nhất với
"không sửa contract đã khóa" và "không bịa".
...
Scanner; `smc_context`/`smc_scorer` KHÔNG nằm trong danh sách xóa Bước 12 — chúng là
producer contract mà Scanner khóa (cùng tầng `indicators`/`technical_context`), không
phải "đường executable scoring". Raw `smc` sẽ gắn `SmcScoringResult` từ
...
- (ai) polynomial freeze: cùng fixture candles → `scanner_features` raws ==
  `signal_engine.*_alignment_score` raw (đến khi xóa).
...
## 7. Điều KHÔNG đổi
- Threshold DEFAULT 40/35/5/2:1 (`scanner-threshold-policy`) — không bịa, không đổi.
- Contract version khóa không đổi.
