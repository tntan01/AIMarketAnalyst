...
- `core.risk_engine` defines 10 module-level constants (including `_TP_SELECTION_AGGRESSIVENESS = 0.5` for conservative TP selection and `_ENTRY_AGGRESSIVENESS = 0.0` for display) for Entry/SL/TP parameters (e.g. `_MIN_STOP_DISTANCE_ATR_MULT = 0.20`, `_WATCH_ZONE_OFFSET_ATR = 0.10`, `_SL_FLOOR_BUFFER_ATR = 0.10`). The previously ambiguous `atr * 0.10` (used for 3 different purposes) is now split into 3 distinct constants: `_ZONE_SL_BUFFER_ATR`, `_WATCH_ZONE_OFFSET_ATR`, `_SL_FLOOR_BUFFER_ATR`.
