...
> **Lô ghi chú Detail — `IMPLEMENTED — WAITING_REVIEW Lô ghi chú Detail` (2026-09-18, trên HEAD `64d9228`):** ghi chú refresh nến rời hàng riêng phía trên Chart và vào **corner widget của thanh tab**, nằm **trên** dòng "Quét lúc …", canh phải, `wordWrap` + trần 400 px, mặc định ẩn (Qt bỏ widget ẩn khỏi layout nên không có vùng trắng và tab không cao thêm). Ba câu copy mới: thành công `Nến: MT5 mới · Kế hoạch/Vị trí: snapshot quét (chưa đánh giá lại).`, không nến mới `Nến: snapshot · Chưa có nến mới.`, lỗi `Nến: snapshot · Không cập nhật được.` — text lỗi thô không còn lên UI; `_candle_refresh_notice()` trả câu hằng, không đọc `snapshot_at`, không dùng đồng hồ nên không mốc nào bị đọc thành "lúc quét". Test mới `tests/test_scanner_detail_notice_layout.py` (9 node, dựng `ScannerDetailScreen` thật trong subprocess) + cập nhật expected của một node Lô D theo copy mới (không skip/xfail, không đổi tên node). Targeted **118 passed** (7 file) và **60 passed** (4 file ảnh hưởng trực tiếp); smoke light/dark exit 0 (28 surface, capture trong `temp/`); full suite **6F / 4757P / 7skip / 16xfail** đối chiếu baseline cùng lệnh tại `64d9228` (**6F / 4738P / 7skip / 16xfail**) — tập failure **trùng nhau từng node**, không có failure mới; `reports/` rỗng cả ba lệnh kiểm. Chi tiết ở [nhật ký tiến độ](smc-implementation-progress.md#lô-ghi-chú-detail--refresh-nến-lên-góc-tab-copy-gọn-2026-09-18--implemented--waiting_review-lô-ghi-chú-detail).

> **Quyết định Tech Lead 2026-09-18:** **Lô UI Chart REVIEW PASS.** Default Chart hiển thị 200 nến với `barSpacing=1.5`; Detail refresh nến ngay khi mở rồi mỗi 30 giây, có guard chống worker MT5 chồng. Đây chỉ là presentation/refresh chart, không thay SMC, kế hoạch hay rollout.

> **Lô UI Chart — `IMPLEMENTED — WAITING_REVIEW Lô UI Chart` (2026-09-18, trên HEAD `64d9228`):** hai thay đổi UI Chart. (1) *Mật độ nến*: `assets/chart/index.html` `_applyDefaultTimeScale` nay `visibleBars = 200`, `barSpacing`/`minBarSpacing = 1.5` (trước 100/3/3) — chỉ trục thời gian, không đụng price scale dọc, OHLC, zoom/pan, Entry/SL/TP, SMC overlay hay H1 execution view. (2) *Refresh nến*: `CANDLE_REFRESH_INTERVAL_SECONDS = 30` thay countdown 5 giây; vẫn refresh ngay khi mở Detail; guard `_candle_fetch_active` nay nằm tại chính chỗ tạo worker nên không thể có hai worker MT5; `_scan_timer` 60 giây không đổi; refresh chỉ merge nến vào Chart. Test mới `tests/test_chart_ui_refresh.py` (10 node, không chờ 30 giây thật, không tạo MT5/thread) + cập nhật expected mật độ của node đang pin giá trị cũ (không skip/xfail, không đổi tên node). Targeted **100 passed** (8 file) và `test_dark_theme_surface_phase2` **7 passed** riêng; smoke light/dark exit 0 (28 surface, capture trong `temp/`); full suite **6F / 4748P / 7skip / 16xfail** đối chiếu baseline cùng lệnh trên worktree sạch tại `64d9228` (**6F / 4738P / 7skip / 16xfail**) — tập failure **trùng nhau từng node**, không có failure mới; `reports/` rỗng cả ba lệnh kiểm. Chi tiết ở [nhật ký tiến độ](smc-implementation-progress.md#lô-ui-chart--nến-nhỏ-hơn-một-nửa--refresh-nến-mỗi-30-giây-2026-09-18--implemented--waiting_review-lô-ui-chart).

> **Quyết định Tech Lead 2026-09-18:** **Task145–147 REVIEW PASS.** Gói F-D-01/02/03 đã đóng: report artifacts không còn trong diff; chart bridge giữ no-op khi không có overlay; carrier `price_vs_zone_detail` không còn nửa vời khi `unknown`. Không mở Task148+, rollout hay auto-entry.

> **Cập nhật Tech Lead 2026-09-18:** Lô D (Task145–147) là `CHANGES_REQUESTED`, chưa được nghiệm thu. Coder chỉ sửa F-D-01 (artefact smoke tracked ngoài scope), F-D-02 (khôi phục no-op `decorate_chart_payload` khi không có `smc_overlay`) và F-D-03 (không publish `price_vs_zone_detail` nửa vời khi `unknown`), rồi dừng chờ review lại.

> **Gói sửa F-D-01/02/03 — `IMPLEMENTED — WAITING_REVIEW Gói sửa F-D-01/02/03` (2026-09-18):** F-D-01 — 46 artefact tracked dưới `reports/scanner/` đã khôi phục về revision nền `1673266`; ba script chưa có đường chuyển nay có cờ `--out-dir PATH` (mặc định không đổi) và toàn bộ smoke/QA của lượt này ghi vào `temp/lod_fix/…`; ba lệnh `git diff --name-only` / `git status --porcelain` / `git ls-files --others --exclude-standard` trên `reports/scanner` **đều rỗng**. F-D-02 — `decorate_chart_payload` trả **nguyên object** cho payload không có `smc_overlay` (không key mới, không copy, không mutate); mọi carrier canonical/historical/corrupted vẫn nhận rule H1 vì `build_full_chart_payload` luôn phát key overlay. F-D-03 — `pair_to_ui_row` publish **cả ba** `{price, entry_low, entry_high}` là `None` và `price_vs_zone="unknown"` khi price thiếu/không dùng được (`bool`/`nan`/`inf`) hoặc thiếu một Entry bound; ca hợp lệ không đổi. Test file Lô D 22 → **35 node**; targeted 82/147/304 passed; full suite **6F / 4738P / 7skip / 16xfail**, sáu node thất bại trùng đúng sáu node ID của baseline tại HEAD (`6F / 4703P / 7skip / 16xfail`), phần tăng passed (+35) đúng bằng số node của file test Lô D. Chi tiết ở [nhật ký tiến độ](smc-implementation-progress.md#gói-sửa-f-d-010203-2026-09-18--implemented--waiting_review-gói-sửa-f-d-010203).

Ngày kế hoạch: 2026-09-10. Cập nhật trạng thái: 2026-09-17. Gate 16/40/56/72/**100/116/128/144** đã APPROVED; task 57–72 hoàn tất; **task 73–79 và 80–91 REVIEW PASS**; **task101–111 REVIEW PASS**; **task112–115 IMPLEMENTED, WAITING_REVIEW task116** → **task116 APPROVED (2026-09-15)**; **task117–120 REVIEW PASS**; **task121–128 REVIEW PASS → Task128 APPROVED (2026-09-16)**; **Task129–138 REVIEW PASS (2026-09-17)**; **Task144 APPROVED (2026-09-17)**. **Lô A — publish `protected_swing` canonical tới consumer và Chart — `IMPLEMENTED — WAITING_REVIEW Lô A` (sau gói sửa F-LA-01):** `protected_swing` được publish theo model canonical (id, level, kind, source event và các mốc thời gian) và Chart vẽ được khi và chỉ khi evidence canonical đầy đủ; đo trên 58 snapshot thật **77/77 layer publish**, `substituted_from = 0`. Review độc lập phát hiện **F-LA-01**: carrier Scanner (`analysis_result.smc_selection`) có thể thay `protected_swing` sau khi persistence block đã chứng nhận (đổi level hợp lệ thành `999.0` vẫn đọc `current` và vẫn bị Chart vẽ). Gói sửa bind trường này vào selection trong `smc_scoring.consumer_contract.sides[side].selection` **tại shared reader boundary** (`core/smc_consumer_contract`), phát `SMC_READ_PROTECTED_SWING_MISMATCH` khi hai bản mâu thuẫn; test qua document bytes trên disk + instance persistence mới chứng minh fail-closed, control corpus giữ 77/77. Lô B/C chưa bắt đầu. Task141 không có bản sao recovery vì người dùng đã waive rõ ràng; không được diễn giải thành backup đã tồn tại. Build không chứa `core.analysis_pipeline`/`core.analysis_engine` được chấp nhận là parity của Scanner GUI hiện hành (không có caller từ `main.py`), không là cam kết hỗ trợ Analyze executable. Bốn quyết định D101-01/D101-02/D107-01/D111-01 đã hoàn tất trong [nhật ký tiến độ](smc-implementation-progress.md#quyết-định-tech-lead-sau-blocker--phạm-vi-tiếp-tục-bắt-buộc-2026-09-14); chi tiết lô persistence/cache/đọc lịch sử/UI ở [nhật ký tiến độ](smc-implementation-progress.md). Chưa rollout production/auto-entry, **chưa gỡ adapter compatibility**. Trạng thái từng task lấy từ nhật ký; phạm vi khối và điều kiện gỡ adapter ở [kế hoạch rà soát trước task73](../smc-pre-task73-architecture-review-plan.md#9-kết-quả-lô-1--snapshot-bản-đồ-và-đề-xuất-phạm-vi).
...
**Cập nhật Tech Lead (2026-09-18): Gói hardening sau Lô C `REVIEW PASS`; rollout vẫn `BLOCKED`.** F-HC-01/02 đã đóng: shared send boundary reload policy owner ở thời điểm dispatch, không fallback policy cũ; mọi payload vẫn bị chặn trong no-rollout (`ORDER_INTENT_ONLY` hoặc `SENDS_REAL_ORDER_NOT_FALSE`), kể cả policy file có `live_order_permitted=true`. Đây không phải approval gửi lệnh thật; mọi cutover cần contract và approval riêng.

**Cập nhật Tech Lead mới nhất (2026-09-17): Lô A `REVIEW PASS`.** F-LA-01 đã được đóng tại shared reader boundary: carrier Scanner khác hoặc inject `protected_swing` không được Chart publish; carrier thiếu field nhận đúng record mà persistence block chứng nhận. **Lô B REVIEW PASS (Mục tiêu 1 BLOCKED hợp lệ, Mục tiêu 2 không xóa adapter).** **Lô B-Ctx `IMPLEMENTED — WAITING_REVIEW Lô B-Ctx`** theo D-LB-01: cache **canonical context** tại injection seam `build_smc_snapshot(context_builder=...)`, nối qua caller Scanner production thực (`_analyze_one_symbol` → `derive_live_analysis`); hit chỉ bỏ context build, `evaluate_smc_snapshot`/coordinator/finalizer vẫn chạy fresh đúng một lần. Đây **không** phải evaluator-cache hit và **không** tuyên bố evaluator không chạy lại. Lô B/C vẫn chưa bắt đầu.

**Review Tech Lead Lô B-Ctx (2026-09-17): `REVIEW PASS` với context-cache production `BLOCKED/DEFERRED`.** F-BCTX-01 đã đóng: workflow `run_market_scan` thật chứng minh không có reuse cùng `symbol + cutoff + candles`; khi cưỡng bức inject seam thì 2 lượt cho 0 hit / 2 miss / 2 file. Vì vậy wiring `context_cache_root=app_data_dir()` đã được gỡ, Scanner không tạo `cache/smc_context` user runtime, identity/revalidation/Analyze không đổi. Seam chỉ là điểm tiêm tường minh cho caller tương lai; chỉ bật production sau khi có reuse thật và Tech Lead chốt retention bounded/observable. Lô C chưa bắt đầu.

**Gói hardening sau Lô C (2026-09-18) — `IMPLEMENTED — WAITING_REVIEW Gói hardening Lô C` (sau gói sửa F-HC-01/02):** review đầu ghi `CHANGES_REQUESTED` với hai BLOCKING. **F-HC-01:** boundary chỉ đọc `_active_order_policy` nạp lúc scan ⇒ nay **reload + validate `RuntimeOrderPolicy` từ owner config ngay trước lệnh gọi broker duy nhất**; config thiếu/hỏng/non-bool ⇒ `ORDER_POLICY_UNAVAILABLE` và **không fallback** sang policy cũ. **F-HC-02:** khi switch `true`, payload intent-only `sends_real_order=False` từng tới được `place_market_order` ⇒ nay boundary **đóng vô điều kiện** trong trạng thái không-rollout: `False` ⇒ `ORDER_INTENT_ONLY`, `True`/thiếu/kiểu sai ⇒ `SENDS_REAL_ORDER_NOT_FALSE`; `live_order_permitted` chỉ còn là **evidence cho cutover tương lai**, không cấp quyền và không làm `place_market_order` tới được. Đã gỡ mọi control "then places" ở test cũ (harness không còn mở rào chắn; các test kiểm chuỗi revalidation/lot/SMC **đến** boundary rồi `place_calls=0`), thêm guard AST quét toàn bộ `tests/` để không test đang chạy nào khẳng định `place_calls == 1`. **Không đổi** F-C-02 timestamp/P10 — test của chúng vẫn xanh nguyên. Rollout vẫn `BLOCKED` và phải là thay đổi/approval riêng, gồm cả contract payload riêng.

**Quyết định Tech Lead Lô B (2026-09-17):** chấp nhận `BLOCKED` của result-cache production và **chọn context cache**, không mở rộng persistence record sang `ScenarioPlan`. Lô B-Ctx được phép cache canonical context qua `build_smc_snapshot(context_builder=...)`; evaluator/coordinator/finalizer vẫn chạy fresh, cache không thay freshness/revalidation. Adapter giữ nguyên; Lô C chưa bắt đầu.

**Ưu tiên hiện tại:** Task116, Task128 và **Task144 APPROVED**. Task139–143 đã được review; Task141 được bàn giao theo waiver recovery rõ ràng của người dùng, **không có bản sao backup**. **Lô A — publish `protected_swing` canonical tới consumer và Chart — `REVIEW PASS` (sau gói sửa F-LA-01).** **Lô B — result-cache production `BLOCKED` hợp lệ, adapter compatibility `REVIEW PASS` (không xóa gì).** **Lô B-Ctx — cache canonical context (`REVIEW PASS`; production `BLOCKED/DEFERRED` sau F-BCTX-01).** **Lô C — discovery source-age/P10/dispatch/rollout — `IMPLEMENTED — WAITING_REVIEW Lô C`:** lô không sửa code; lập map ba owner freshness (snapshot SLA 120s/30s; gate safety với SLA candle 3 phút / connectivity 5 phút từ `config/scanner_order_policy.json`; tuổi tick dispatch 30s), map P10/M15 kèm hai mâu thuẫn (tên hằng `_M15_DISPLACEMENT_WINDOW` trong bảng tham số; phạm vi max-run trước/hậu confirmation), audit dispatch clock (naive bị coerce ngầm, non-datetime ném exception) và audit rollout (5 site `order_send`; `order_enabled` đang `True` và không được đọc ở dispatch). Sáu việc cần Tech Lead chốt đã ghi ở nhật ký. **Gói hardening Lô C — `REVIEW PASS` (2026-09-18), rollout vẫn `BLOCKED`.** **Lô D — Task145–147 (H1 execution view + "Vị trí" snapshot + nến refresh) — `IMPLEMENTED — WAITING_REVIEW Lô D`:** H1 chỉ vẽ nến/giá + Entry + SL/TP (không vùng SMC/caption/trigger/protected swing); H4/D1/M15 giữ nguyên; "Vị trí" giữ ngữ nghĩa giá lúc quét và tooltip nêu số thật; nến refresh không đổi kế hoạch. Không thay Entry/SL/TP, không đổi verdict canonical/persistence. **Gói hardening sau Lô C — `IMPLEMENTED — WAITING_REVIEW Gói hardening Lô C`:** đóng F-C-01 (kill switch `live_order_permitted` + rào chắn ngay trước lệnh gọi broker duy nhất; config giao vẫn `false`; payload phải khai `sends_real_order is False`) và F-C-02 (`now`/`tick_time` naive hoặc non-datetime ⇒ mã riêng + `checked_at=None`, không coerce/không ném; candle naive ở safety ⇒ verdict có reason). P10 đã đồng bộ tài liệu và khoá characterisation cả hai nhánh; ngưỡng không đổi. Coder **chưa** làm lô tiếp theo. Build chỉ được chấp thuận cho Scanner GUI hiện hành; không tự mở rộng sang Analyze executable, rollout hay auto-entry. Không mở lại D101-01/D101-02/D107-01/D111-01. **Lô D (2026-09-18) — `IMPLEMENTED — WAITING_REVIEW Lô D`:** theo chỉ đạo người dùng, **H1 là execution view** — Chart H1 chỉ vẽ nến/giá + band Entry của kế hoạch + SL/TP, không vùng SMC, không caption, không M15 trigger, không protected swing, kể cả khi payload canonical có đủ dữ liệu; H4/D1/M15 giữ nguyên. Cột **"Vị trí"** giữ đúng ngữ nghĩa snapshot (so **giá lúc quét** với `ScenarioPlan.entry_zone` của side đã chọn) và tooltip nay nói rõ nó đo cái gì kèm số thật; thiếu giá/band ⇒ `Không xác định`, không suy diễn thành "Trong vùng". **Nến refresh** không đổi `price_vs_zone`/kế hoạch, thông báo nêu snapshot mà kế hoạch thuộc về và không bịa mốc thời gian. Đây là **thay đổi trình bày**: verdict canonical/persistence không đổi, không đọc `selected_zone` legacy, không đổi schema. Chi tiết ở [nhật ký tiến độ](smc-implementation-progress.md#lô-d--task145147-h1-là-execution-view-vị-trí-là-ngữ-nghĩa-snapshot-nến-refresh-không-đổi-kế-hoạch-2026-09-18--implemented--waiting_review-lô-d). Checklist 144 task bên dưới giữ làm phạm vi phương án đầy đủ; số task, yêu cầu và mốc review không đổi.
...
| 116 | TECH LEAD REVIEW — DỪNG | Trình flow thực tế, parity và ca blocked/unknown/fallback/revalidation. | Chỉ làm task 117 khi task 116 được APPROVED; chưa cho auto-entry dùng build đang nghiệm thu. **ĐÃ APPROVED (2026-09-15).** |
| 117 | services/scanner_persistence_service.py | Lưu các trường result/visit/confirmation/provenance cần đọc lại. | Round-trip không mất raw/null/reason/source; không cần database riêng. **REVIEW PASS 2026-09-16:** Scanner/Analyze cùng persistence block, restart đã kiểm. |
| 118 | Cache SMC / metadata | Cập nhật dấu tương thích cache và chứng nhận cấu hình bị ảnh hưởng. | Cache/chứng nhận cũ không tự hợp lệ với logic mới; không đổi nhãn để lách policy. **REVIEW PASS 2026-09-16.** |
| 119 | Persistence readers | Giữ đọc lịch sử theo ý nghĩa dữ liệu cũ. | Không biến điểm cũ thành điểm mới; không xóa journal hoặc sửa lệnh đang mở. **REVIEW PASS 2026-09-16.** |
| 120 | tests persistence / cache | Kiểm tra payload cũ/mới, cache miss, corrupted data và restart. | Lịch sử mở được; payload không tương thích không trở thành kết quả live hợp lệ. **REVIEW PASS 2026-09-16:** có Scanner caller → disk → instance mới. |
...
| 128 | TECH LEAD REVIEW — DỪNG | Trình persistence round-trip, ảnh UI và ví dụ người dùng đọc lý do chờ. | **APPROVED 2026-09-16.** Được lập/giao Task129+ theo lô riêng; Tech Lead đã duyệt độ đơn giản và việc giữ lịch sử. |
...
| Task | File / phạm vi | Việc Coder cần làm | Đầu ra / điều kiện hoàn thành |
|---|---|---|---|
| 129 | Golden fixtures | Cập nhật expected của các fixture có hành vi đổi đã được duyệt. | Mỗi khác biệt có lý do; giữ các invariant không thuộc SMC. **KHÔNG sửa expected nào trong lô:** golden chạy xanh 76 passed; các diff golden đã có (thuộc `c42770e`, đã duyệt) được ghi mapping kèm lý do trong nhật ký. |
...
| 131 | Snapshot thực tế | Thu 20–30 snapshot từ symbol thường dùng bằng nguồn có sẵn. | Có dữ liệu/cutoff và đủ nhóm tăng/giảm/range, vùng tốt/hỏng, thiếu/đủ M15; không dùng synthetic để giả dữ liệu thật. **58 snapshot thật từ MT5** (55 trên symbol cấu hình + 3 row data-quality theo broker symbol), cutoff + provenance + digest, ma trận đủ nhóm. |
| 132 | QA chart thực tế | Đối chiếu các snapshot task 131 với vùng, protected swing và trạng thái. | Ghi expected/observed, nhận xét từng ca; không chỉ chụp ảnh không đánh giá. `protected_swing` giữ `SMC_PROTECTED_SWING_UNAVAILABLE`, không thay bằng SL/legacy. |
...
| 137 | Sửa lỗi nghiệm thu | Xử lý lần lượt lỗi mới trong nhật ký nghiệm thu; mỗi bản sửa gắn task gốc và nguyên nhân. | Không gộp thay mô hình ngoài scope; sửa rule đã duyệt phải qua review lại gate chịu ảnh hưởng. **ĐÃ LÀM:** tối ưu tái dùng trong canonical structural evaluation (F129-136-01), giữ raw technical đã derive qua composition (F129-136-02), fixture UI tất định (F129-136-03) — chi tiết ở nhật ký. |
| 138 | Kiểm tra lại sau sửa | Chạy lại test bị ảnh hưởng và kiểm tra tích hợp tương ứng. | Không cần lặp toàn bộ suite nếu không có lý do; ghi evidence sau bản sửa cuối. **ĐÃ LÀM:** targeted + equivalence + replay/persistence parity + scanner_smoke + benchmark + full suite (`6F/4549P/7skip/16xfail`, đối chiếu baseline từng node). |
| 139 | Tài liệu sử dụng | Viết hướng dẫn ngắn đọc score, vùng, chờ xác nhận và thiếu dữ liệu. | Không có nhãn số thế hệ hoặc cách chỉnh tham số hàng loạt. **ĐÃ LÀM:** thêm §3.2 vào `docs/guides/USER_GUIDE.md` (tài liệu người dùng đang dùng, cập nhật tại chỗ — không tạo bản trùng). Đối chiếu **ngược từ code**: 14/14 nhãn bảng của panel Detail và 23/23 mục từ vựng trạng thái/vòng đời/xác nhận có mặt đúng chữ; 9/9 câu lý do khớp nguyên văn `SMC_REASON_TEXT`. Lượt đầu thiếu 5 nhãn + 8 mục và lộ lỗi gộp nhầm ba ô trạng thái — đã sửa. Không có version/cache key/contract version/công thức B-Q-L-C/hướng dẫn chỉnh ngưỡng hàng loạt trên bề mặt người dùng. |
| 140 | Dọn đường chạy | Rà caller cũ, cap/phạt ngầm, scorer trùng và imports không còn dùng. | Chỉ một công thức live; giữ reader lịch sử nếu cần, không xóa file ngoài scope. **ĐÃ LÀM — KHÔNG XÓA GÌ:** bảng caller inventory đầy đủ cho `score_smc`, `build_smc_context`, `_smc_for_timeframe`, canonical snapshot/evaluator, `read_canonical_selection`, `scenario_preferred_zone_for_side`, `selected_zone*`, cache/persistence reader, projection Scanner/Analyze/replay. Ứng viên duy nhất tưởng dead (`build_smc_context` import ở `core/analysis_pipeline.py:49`) **không** dead: là đích monkeypatch của `scripts/tier2_feasibility_gate.py:227,243,257` ⇒ xóa sẽ `AttributeError`. Mọi ứng viên khác còn đường chạy hoặc là seam/adapter/historical reader ⇒ `DEFERRED` kèm lý do. Không đổi tên nhãn, không bỏ assertion/test, không thay golden, không thêm skip/xfail. |
| 141 | Khôi phục / chuyển đổi | Chuẩn bị build nền, bản sao cấu hình/dữ liệu liên quan và quy trình khôi phục. | Không xóa lịch sử, không chỉnh lệnh mở; xác định cache nào được làm mới. **ĐÃ LÀM:** inventory 7 nhóm (mã/build, config, persistence documents, journal/lệnh đang mở, corpus & artifact bằng chứng, cache tái tạo được, cache **không** phải nguồn sự thật) + quy trình recovery 8 bước theo thứ tự bắt buộc. Ghi rõ persistence documents **không phải archive** (retention xóa sau 24 h / 7 ngày) và `cache/smc_results/` chưa tồn tại. **`BLOCKED: backup destination not authorized`** — chưa có vị trí backup được người dùng phê duyệt; checklist và dry-run read-only trên temp storage vẫn hoàn thành. |
| 142 | Build kiểm tra cục bộ | Tạo/chạy build hoặc cách khởi động ứng dụng hiện dùng; kiểm tra import và màn hình chính. | Artifact sẵn sàng bàn giao; chưa tự thay bản đang dùng hoặc bật auto-trade trước review cuối. **ĐÃ LÀM:** compileall exit 0; import 15/15; `MainWindow` boot cả hai theme, 7/7 route không raise; `smc_ui_smoke` exit 0 (7 trạng thái × 2 theme); `scanner_smoke` exit 0 (cả `SMOKE` và `PATHB`); restart 4/4; replay 116 ca `no_future_leak=True`; PyInstaller exit 0 (`.exe` 22.650.649 B) và boot `.exe` offscreen sống 25 s không traceback. **BLOCKED:** quét thật từ UI (không có MT5/broker) và nội dung Settings/Journal với dữ liệu thật. |
| 143 | Hồ sơ bàn giao | Tổng hợp task đã làm, review, test, snapshot QA, performance và tồn tại. | Phân biệt xác minh kỹ thuật với hiệu quả giao dịch chưa xác lập; không giấu task BLOCKED. **ĐÃ LÀM:** cập nhật tại chỗ `smc-acceptance-dossier.md` (§8–§16: snapshot/baseline/diff boundary, mapping caller, kết quả Task129–138, review/finding đã đóng, deferred/blocked, recovery checklist, command build/run, phân biệt xác minh kỹ thuật với hiệu quả giao dịch, 8 điều kiện cho Task144) và nhật ký tiến độ. Không tạo báo cáo phụ. |
...
| 145 | Execution view cho H1 | Chart H1 chỉ hiển thị nến/giá + Entry + SL/TP; không vùng SMC, caption, M15 trigger hay protected swing. | Không thay Entry bằng band SMC, không tính lại Entry/SL/TP, không đổi side/plan/risk/scoring/gate; H4/D1/M15 giữ nguyên hành vi. Luật là metadata trình bày additive (`execution_view_timeframes`) đi kèm payload. **ĐÃ LÀM:** `ui/chart_bridge.EXECUTION_VIEW_TIMEFRAMES` + `decorate_chart_payload`; `_isExecutionView` trong `assets/chart/index.html` (`_smcLayer` trả `null` ⇒ `_renderSmcOverlay` early-return, source zone cũng không vẽ). Evidence: `smc_ui_smoke` H1 rỗng caption ở 7 trạng thái × 2 theme trong khi payload còn dữ liệu; control bỏ luật ⇒ caption hiện lại; QA thật giữ caption H4. |
| 146 | "Vị trí" theo ngữ nghĩa snapshot | Cột so giá lúc quét với Entry của kế hoạch đã chọn; tooltip phải nói rõ và nêu số thật. | Không nêu H1/H4 hay vùng SMC trong execution view; thiếu giá/band ⇒ `Không xác định`, tuyệt đối không suy ra "Trong vùng". **ĐÃ LÀM:** `price_vs_zone_detail` (`price`/`entry_low`/`entry_high`/`snapshot_at`) phát từ `core/scanner_ui_adapter.pair_to_ui_row` (dòng blocked nhận bản toàn `None`); `ScannerTableModel._price_vs_zone_tooltip` + `_price_vs_zone_reading`. Evidence: `Giá lúc quét 1.23790 nằm trong Entry 1.23710–1.23913.` |
| 147 | Nến refresh không đổi kế hoạch | Merge nến mới từ MT5 chỉ đổi phần nến; kế hoạch và cột "Vị trí" vẫn thuộc snapshot. | Không re-score/re-select, không đổi `price_vs_zone`, không nói/ám chỉ đã đánh giá lại, không bịa mốc thời gian; thiếu provenance ⇒ "snapshot thời điểm quét". **ĐÃ LÀM:** `ScannerDetailScreen._candle_refresh_notice` đọc `snapshot_at` của dòng (kiểm bằng AST: không `now`/`utcnow`/`today`). Evidence: merge nến ⇒ `price_vs_zone`, `price_vs_zone_detail` và `analysis_result` không đổi. |
